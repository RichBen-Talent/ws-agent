...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/ws-agent/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/ws-agent");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/ws-agent/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__WS_AGENT_CONTROL_UI_BASE_PATH__ = "/ws-agent";
    const app = mountApp("/ws-agent/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/ws-agent");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/ws-agent/sessions");
  });
