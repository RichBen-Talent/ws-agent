...
import type { WsAgentApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as WsAgentApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as WsAgentApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as WsAgentApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as WsAgentApp);
    await loadConfig(host as unknown as WsAgentApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as WsAgentApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as WsAgentApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as WsAgentApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as WsAgentApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as WsAgentApp);
    await loadDevices(host as unknown as WsAgentApp);
    await loadConfig(host as unknown as WsAgentApp);
    await loadExecApprovals(host as unknown as WsAgentApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as WsAgentApp);
    await loadConfig(host as unknown as WsAgentApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as WsAgentApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as WsAgentApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__WS_AGENT_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  const app = host as unknown as WsAgentApp;
...
async function loadOverviewLogs(host: WsAgentApp) {
...
function buildAttentionItems(host: WsAgentApp) {
  ...
  if (auth?.scopes && !auth.scopes.includes("operator.read")) {
    items.push({
      ...
      href: "https://docs.ws-agent.ai/web/dashboard",
      external: true,
    });
  }
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as WsAgentApp, true),
    loadConfigSchema(host as unknown as WsAgentApp),
    loadConfig(host as unknown as WsAgentApp),
  ]);
}

export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as WsAgentApp, false),
    loadCronStatus(host as unknown as WsAgentApp),
    loadCronJobs(host as unknown as WsAgentApp),
  ]);
}
