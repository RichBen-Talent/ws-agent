...
export async function handleWhatsAppStart(host: WsAgentApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: WsAgentApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: WsAgentApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: WsAgentApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: WsAgentApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: WsAgentApp): string {
...
function resolveGatewayHttpAuthHeader(host: WsAgentApp): string | null {
...
function buildGatewayHttpHeaders(host: WsAgentApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: WsAgentApp) {
...
export function handleNostrProfileToggleAdvanced(host: WsAgentApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: WsAgentApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: WsAgentApp) {
  const state = host.nostrProfileFormState;
