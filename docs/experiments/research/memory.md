...
read_when:
  - Designing workspace memory (~/.ws-agent/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep WsAgent integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.ws-agent/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.ws-agent/workspace` as canonical human-readable memory.
...
```
~/.ws-agent/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in WsAgent**, but keep a separable core library.

### Why integrate into WsAgent?

- WsAgent already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `ws-agent memory recall "…" --k 25 --since 30d`
  - `ws-agent memory reflect --since 7d`
...
Pragmatic take for `~/.ws-agent/workspace`:
