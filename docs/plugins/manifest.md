...
read_when:
  - You are building a WsAgent plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (ws-agent.plugin.json)

Every plugin **must** ship a `ws-agent.plugin.json` file in the **plugin root**.
WsAgent uses this manifest to validate configuration **without executing plugin
