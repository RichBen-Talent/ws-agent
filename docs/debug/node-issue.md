...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in WsAgent
...
Running WsAgent via Node with `tsx` fails at startup with:

```
[ws-agent] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `ws-agent status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch ws-agent.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node ws-agent.mjs status` works on Node 25.
