...
Deploy WsAgent on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/ws-agent-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/ws-agent` — Control UI

## What you get

- Hosted WsAgent Gateway + Control UI
...
- `WS_AGENT_STATE_DIR=/data/.ws-agent` (recommended)
- `WS_AGENT_WORKSPACE_DIR=/data/workspace` (recommended)
- `WS_AGENT_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your WsAgent state + workspace so you can migrate to another host without losing config or memory.
