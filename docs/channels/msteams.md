...
```bash
ws-agent plugins install @ws-agent/msteams
```

Local checkout (when running from a git repo):

```bash
ws-agent plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
WsAgent will offer the local install path automatically.
...
3. Configure WsAgent with those credentials.
...
- Talk to WsAgent via Teams DMs, group chats, or channels.
...
- On startup, WsAgent resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.ws-agent/ws-agent.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring WsAgent, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `ws-agent-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `ws-agent plugins install @ws-agent/msteams`
   - From a local checkout: `ws-agent plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "WsAgent in Teams", "full": "WsAgent in Teams" },
...
WsAgent handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, WsAgent only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/WsAgentShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

WsAgent sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `ws-agent message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.ws-agent/msteams-polls.json`.
...
```bash
ws-agent message send --channel msteams \
...
ws-agent message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
ws-agent message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
ws-agent message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
ws-agent message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
