...
```bash
ws-agent plugins install @ws-agent/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
ws-agent plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "WsAgent" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `ws-agent pairing list nextcloud-talk`
  - `ws-agent pairing approve nextcloud-talk <CODE>`
