...
Tlon is a decentralized messenger built on Urbit. WsAgent connects to your Urbit ship and can
...
```bash
ws-agent plugins install @ws-agent/tlon
```

Local checkout (when running from a git repo):

```bash
ws-agent plugins install ./extensions/tlon
```
...
By default, WsAgent blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `ws-agent message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, WsAgent replies in-thread.
