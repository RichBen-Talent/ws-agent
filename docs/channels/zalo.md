...
- Install via CLI: `ws-agent plugins install @ws-agent/zalo`
...
1. Install the Zalo plugin:
   - From a source checkout: `ws-agent plugins install ./extensions/zalo`
   - From npm (if published): `ws-agent plugins install @ws-agent/zalo`
...
- Approve via:
  - `ws-agent pairing list zalo`
  - `ws-agent pairing approve zalo <CODE>`
...
- Example: `ws-agent message send --channel zalo --target 123456789 --message "hi"`.

## Troubleshooting

**Bot doesn't respond:**

- Check that the token is valid: `ws-agent channels status --probe`
- Verify the sender is approved (pairing or allowFrom)
- Check gateway logs: `ws-agent logs --follow`
