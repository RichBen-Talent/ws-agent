...
read_when:
  - You want WsAgent to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables WsAgent to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`ws-agent onboard`) and `ws-agent channels add` list optional channel plugins.
...
```bash
ws-agent plugins install @ws-agent/nostr
```

Use a local checkout (dev workflows):

```bash
ws-agent plugins install --link <path-to-ws-agent>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "ws-agent@example.com",
        "lud16": "ws-agent@example.com"
      }
    }
  }
}
