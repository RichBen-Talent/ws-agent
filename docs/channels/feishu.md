...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects WsAgent to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
ws-agent plugins install @ws-agent/feishu
```

Local checkout (when running from a git repo):

```bash
ws-agent plugins install ./extensions/feishu
```
...
If you just installed WsAgent, run the wizard:
...
2. Configuring app credentials in WsAgent
...
1. You already ran `ws-agent channels add` for Feishu
2. The gateway is running (`ws-agent gateway status`)
...
```bash
ws-agent pairing approve feishu <CODE>
```
...
2. Run `ws-agent logs --follow` and look for `chat_id`
...
2. Run `ws-agent logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `ws-agent gateway status`  | Show gateway status           |
| `ws-agent gateway install` | Install/start gateway service |
| `ws-agent gateway stop`    | Stop gateway service          |
| `ws-agent gateway restart` | Restart gateway service       |
| `ws-agent logs --follow`   | Tail gateway logs             |
...
4. Check logs: `ws-agent logs --follow`
...
5. Ensure the gateway is running: `ws-agent gateway status`
6. Check logs: `ws-agent logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.ws-agent/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.ws-agent/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
