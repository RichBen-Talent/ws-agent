...
Matrix is an open, decentralized messaging protocol. WsAgent connects as a Matrix **user**
...
```bash
ws-agent plugins install @ws-agent/matrix
```

Local checkout (when running from a git repo):

```bash
ws-agent plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `ws-agent plugins install @ws-agent/matrix`
   - From a local checkout: `ws-agent plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: WsAgent calls the same
     login endpoint, stores the access token in `~/.ws-agent/credentials/matrix/credentials.json`,
...
- On first connection, WsAgent requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  WsAgent logs a warning.
...
`~/.ws-agent/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `ws-agent pairing list matrix`
  - `ws-agent pairing approve matrix <CODE>`
...
- On startup, WsAgent resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
