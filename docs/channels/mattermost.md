---
summary: "Mattermost bot setup and WsAgent config"
read_when:
...
```bash
ws-agent plugins install @ws-agent/mattermost
```

Local checkout (when running from a git repo):

```bash
ws-agent plugins install ./extensions/mattermost
```
...
4. Configure WsAgent and start the gateway.
...
- Approve via:
  - `ws-agent pairing list mattermost`
  - `ws-agent pairing approve mattermost <CODE>`
...
Use these target formats with `ws-agent message send` or cron/webhooks:
