...
read_when:
  - Setting up Twitch chat integration for WsAgent
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. WsAgent connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
ws-agent plugins install @ws-agent/twitch
```

Local checkout (when running from a git repo):

```bash
ws-agent plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `WS_AGENT_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "ws-agent", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use WS_AGENT_TWITCH_ACCESS_TOKEN env var)
...
```bash
WS_AGENT_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
