...
- `gateway`/`node` approvals are controlled by `~/.ws-agent/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  WsAgent prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`ws-agent security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `ws-agent doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
