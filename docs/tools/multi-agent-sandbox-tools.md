...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `ws-agent sandbox explain`.
...
Legacy `agent.*` configs are migrated by `ws-agent doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=ws-agent-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${WS_AGENT_STATE_DIR:-$HOME/.ws-agent}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
