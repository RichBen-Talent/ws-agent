---
summary: "Agent tool surface for WsAgent (browser, canvas, nodes, message, cron) replacing legacy `ws-agent-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `ws-agent-*` skills
title: "Tools"
---

# Tools (WsAgent)

WsAgent exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `ws-agent-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `ws-agent.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, WsAgent logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:ws-agent`: all built-in WsAgent tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`ws-agent node run`).
...
WsAgent tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `ws-agent configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated WsAgent-managed browser.
...
- Quick smoke: `ws-agent nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `ws-agent gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, WsAgent clamps `tools.sessions.visibility` to `tree`.
