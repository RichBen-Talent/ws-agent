---
summary: "Chrome extension: let WsAgent drive your existing Chrome tab"
read_when:
...
The WsAgent Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate ws-agent-managed Chrome profile.
...
WsAgent then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the WsAgent release (npm package) as static files. There is no separate “build” step.

After upgrading WsAgent:

- Re-run `ws-agent browser extension install` to refresh the installed files under your WsAgent state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

WsAgent ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `WS_AGENT_GATEWAY_TOKEN`)
...
- Open the tab you want WsAgent to control.
...
- `ON`: attached; WsAgent can drive that tab.
...
Debugging: `ws-agent sandbox explain`
...
`ws-agent browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `ws-agent browser extension install` first to copy the extension to a stable location under your WsAgent state directory.
...
- **This is not isolated** like the dedicated ws-agent-managed profile.
