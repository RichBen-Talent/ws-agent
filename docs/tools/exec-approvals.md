...
- **gateway host** → `ws-agent` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.ws-agent/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.ws-agent/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `ws-agent approvals allowlist ...`).
- `ws-agent security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `ws-agent doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.ws-agent/exec-approvals.json` directly.

CLI: `ws-agent approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
