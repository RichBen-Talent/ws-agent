---
summary: "Use Synthetic's Anthropic-compatible API in WsAgent"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. WsAgent registers it as the
...
Note: WsAgent's Anthropic client appends `/v1` to the base URL, so use
