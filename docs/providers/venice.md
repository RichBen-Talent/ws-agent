---
summary: "Use Venice AI privacy-focused models in WsAgent"
read_when:
  - You want privacy-focused inference in WsAgent
...
```bash
ws-agent onboard --auth-choice venice-api-key
```
...
```bash
ws-agent chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, WsAgent shows all available Venice models. Pick based on your needs:
...
```bash
ws-agent models set venice/claude-opus-45
ws-agent models set venice/llama-3.3-70b
```

List all available models:

```bash
ws-agent models list | grep venice
```

## Configure via `ws-agent configure`

1. Run `ws-agent configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
WsAgent automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
ws-agent chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
ws-agent chat --model venice/claude-opus-45

# Use uncensored model
ws-agent chat --model venice/venice-uncensored

# Use vision model with image
ws-agent chat --model venice/qwen3-vl-235b-a22b

# Use coding model
ws-agent chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `ws-agent models list` to see currently available models. Some models may be temporarily offline.
