---
summary: "Use MiniMax M2.1 in WsAgent"
read_when:
  - You want MiniMax models in WsAgent
...
```bash
ws-agent plugins enable minimax-portal-auth  # skip if already loaded.
ws-agent gateway restart  # restart if gateway is already running
ws-agent onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/ws-agent/ws-agent/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `ws-agent.json`:
...
- Use `ws-agent models list` and `ws-agent models set minimax/MiniMax-M2.1` to switch.
...
- Running `ws-agent configure` and selecting **MiniMax M2.1**, or
