...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/ws-agent`)
...
If the page fails to load, start the Gateway first: `ws-agent gateway`.
...
you revoke it with `ws-agent devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.ws-agent/ws-agent.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. WsAgent
...
WsAgent **blocks** Control UI connections without device identity.
