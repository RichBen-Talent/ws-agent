...
```bash
ws-agent tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `ws-agent tui --deliver`
...
- Check the Gateway logs: `ws-agent logs --follow`.
- Confirm the agent can run: `ws-agent status` and `ws-agent models status`.
...
- No agents in picker: check `ws-agent agents list` and your routing config.
