---
summary: "WsAgent on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up WsAgent on a Raspberry Pi
  - Running WsAgent on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# WsAgent on Raspberry Pi

## Goal

Run a persistent, always-on WsAgent Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most WsAgent features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled ws-agent

# Enable if not
sudo systemctl enable ws-agent

# Start on boot
sudo systemctl start ws-agent
```
...
```bash
# Check logs
journalctl -u ws-agent --no-pager -n 100

# Common fix: rebuild
cd ~/ws-agent  # if using hackable install
npm run build
sudo systemctl restart ws-agent
```
