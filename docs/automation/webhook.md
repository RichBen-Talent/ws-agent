...
read_when:
  ...
  - Wiring external systems into WsAgent
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your WsAgent config directory (typically `~/.ws-agent/hooks/transforms`).
...
- `ws-agent webhooks gmail setup` writes `hooks.gmail` config for `ws-agent webhooks gmail run`.
