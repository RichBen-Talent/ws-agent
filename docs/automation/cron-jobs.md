...
- Jobs persist under `~/.ws-agent/cron/` so restarts don’t lose schedules.
...
ws-agent cron list
ws-agent cron run <job-id>
ws-agent cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.ws-agent/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `ws-agent cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, WsAgent applies a
...
If `delivery` is omitted for isolated jobs, WsAgent defaults to `announce`.
...
- Job store: `~/.ws-agent/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.ws-agent/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
ws-agent cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
ws-agent cron edit <jobId> --agent ops
ws-agent cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
ws-agent cron run <jobId>
ws-agent cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
ws-agent cron edit <jobId> \
...
```bash
ws-agent cron edit <jobId> --exact
```
...
```bash
ws-agent system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`ws-agent system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `WS_AGENT_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- WsAgent applies exponential retry backoff for recurring jobs after consecutive errors:
