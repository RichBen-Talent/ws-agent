...
description: "Complete field-by-field reference for ~/.ws-agent/ws-agent.json"
---

# Configuration Reference

Every field available in `~/.ws-agent/ws-agent.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — WsAgent uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `ws-agent doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["ws-agent-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `ws-agent plugins install @ws-agent/mattermost`.
...
WsAgent spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@ws-agent"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `ws-agent.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, WsAgent auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, WsAgent assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          network: "ws-agent-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.ws-agent/sandboxes`
...
noVNC observer access uses VNC auth by default and WsAgent emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `ws-agent-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        agentDir: "~/.ws-agent/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@ws-agent"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:ws-agent`   | All built-in tools (excludes provider plugins)                                           |
...
WsAgent uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.ws-agent/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `WS_AGENT_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `ws-agent onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `ws-agent onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `ws-agent onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `ws-agent onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `ws-agent onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `ws-agent onboard --auth-choice minimax-api`.
...
- Loaded from `~/.ws-agent/extensions`, `<workspace>/.ws-agent/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or WS_AGENT_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `WS_AGENT_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.ws-agent-dev` + port `19001`), `--profile <name>` (uses `~/.ws-agent-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.ws-agent/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-ws-agent-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `WS_AGENT_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or WS_AGENT_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__wsAgent__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__wsAgent__/a2ui/`
...
- Also serves A2UI at `/__wsAgent__/a2ui/`.
...
- Hostname defaults to `ws-agent`. Override with `WS_AGENT_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.ws-agent/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `ws-agent dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.ws-agent/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${WS_AGENT_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.ws-agent/credentials/oauth.json`.
...
- Default log file: `/tmp/ws-agent/ws-agent-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `ws-agent doctor --fix` can strip unknown keys).
