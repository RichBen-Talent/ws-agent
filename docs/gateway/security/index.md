...
## Quick check: `ws-agent security audit`
...
```bash
ws-agent security audit
ws-agent security audit --deep
ws-agent security audit --fix
ws-agent security audit --json
```

It flags common footguns (Gateway auth exposure, browser control exposure, elevated allowlists, filesystem permissions).

WsAgent is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
WsAgent assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.ws-agent`, including `ws-agent.json`), treat them as a trusted operator.
...
If you run `--deep`, WsAgent also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.ws-agent/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.ws-agent/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.ws-agent/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.ws-agent/credentials/oauth.json`
...
| -------------------------------------------------- | ------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                | critical      | Other users/processes can modify full WsAgent state                                | filesystem perms on `~/.ws-agent`                                                                 | yes      |
| `fs.config.perms_writable`                         | critical      | Others can change auth/tool policy/config                                          | filesystem perms on `~/.ws-agent/ws-agent.json`                                                   | yes      |
...
`ws-agent security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`ws-agent security audit` includes `config.insecure_or_dangerous_flags` when any
...
gateway:
  ...
  auth:
    mode: password
    password: ${WS_AGENT_GATEWAY_PASSWORD}
```
...
WsAgent stores session transcripts on disk under `~/.ws-agent/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.ws-agent` (see the audit section below). If you need
...
WsAgent can refresh the skills list mid-session:
...
- If you install plugins from npm (`ws-agent plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.ws-agent/extensions/<pluginId>/` (or `$WS_AGENT_STATE_DIR/extensions/<pluginId>/`).
  - WsAgent uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
ws-agent pairing list <channel>
ws-agent pairing approve <channel> <code>
```
...
By default, WsAgent routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
WsAgent has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.ws-agent/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.ws-agent or your logs.”

## Unsafe external content bypass flags

WsAgent includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.ws-agent/ws-agent.json`: `600` (user read/write only)
- `~/.ws-agent`: `700` (user only)

`ws-agent doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `WS_AGENT_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__wsAgent__/canvas/` and `/__wsAgent__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_ws-agent-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `WS_AGENT_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `ws-agent doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `WS_AGENT_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `WS_AGENT_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), WsAgent
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. WsAgent verifies the identity by resolving the
...
- WsAgent will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.ws-agent/` (or `$WS_AGENT_STATE_DIR/`) may contain secrets or private data:

- `ws-agent.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `ws-agent status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@ws-agent", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.ws-agent/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `ws-agent` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only WsAgent clients can connect.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default WsAgent limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `ws-agent gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `WS_AGENT_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/ws-agent/ws-agent-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.ws-agent/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `ws-agent security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + WsAgent version
...
Found a vulnerability in WsAgent? Please report responsibly:

1. Email: [security@ws-agent.ai](mailto:security@ws-agent.ai)
