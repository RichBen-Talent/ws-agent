...
read_when:
  - Running WsAgent behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of WsAgent
...
- You run WsAgent behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. WsAgent checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. WsAgent extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://ws-agent.example.com
    to: http://ws-agent-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
location / {
    ...
    proxy_pass http://ws-agent:18789;
...
`ws-agent security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update WsAgent config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `ws-agent security audit` and review findings
