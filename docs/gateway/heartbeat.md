...
- During heartbeat runs, WsAgent treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `ws-agent sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, WsAgent skips the heartbeat run entirely (no model call).
...
```bash
ws-agent system event --text "Check for urgent follow-ups" --mode now
```
