...
`ws-agent doctor` is the repair + migration tool for WsAgent. It fixes stale
...
- Extra workspace dir detection (`~/ws-agent`).
...
you to run `ws-agent doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.ws-agent/ws-agent.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.ws-agent/sessions/` to `~/.ws-agent/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.ws-agent/agent/` to `~/.ws-agent/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.ws-agent/credentials/*.json` (except `oauth.json`)
  - to `~/.ws-agent/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `ws-agent doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.ws-agent` folders exist across
  home directories or when `WS_AGENT_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.ws-agent/ws-agent.json` is
...
offers to remove them and install the WsAgent service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named WsAgent gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `ws-agent doctor --generate-gateway-token` to force token
...
- `ws-agent doctor` prompts before rewriting supervisor config.
- `ws-agent doctor --yes` accepts the default repair prompts.
- `ws-agent doctor --repair` applies recommended fixes without prompts.
- `ws-agent doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `ws-agent gateway install --force`.
