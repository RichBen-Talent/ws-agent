...
This guide summarizes a sane workflow for working on the pi integration in WsAgent.
...
- Trigger the agent directly:
  - `pnpm ws-agent agent --message "Hello" --thinking low`
...
State lives under the WsAgent state directory. Default is `~/.ws-agent`. If `WS_AGENT_STATE_DIR` is set, use that directory instead.

To reset everything:

- `ws-agent.json` for config
...
- [https://docs.ws-agent.ai/testing](https://docs.ws-agent.ai/testing)
- [https://docs.ws-agent.ai/start/getting-started](https://docs.ws-agent.ai/start/getting-started)
