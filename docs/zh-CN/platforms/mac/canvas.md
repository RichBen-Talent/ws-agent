...
- `~/Library/Application Support/WsAgent/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `ws-agent-canvas://<session>/<path>`

示例：

- `ws-agent-canvas://main/` → `<canvasRoot>/main/index.html`
- `ws-agent-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `ws-agent-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
ws-agent nodes canvas present --node <id>
ws-agent nodes canvas navigate --node <id> --url "/"
ws-agent nodes canvas eval --node <id> --js "document.title"
ws-agent nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__wsAgent__/a2ui/
```
...
ws-agent nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
ws-agent nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "ws-agent://agent?message=Review%20this%20design";
```
