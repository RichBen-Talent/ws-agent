...
你可以在 `~/.ws-agent/ws-agent.json` 中覆盖此设置：
...
所有日志配置都在 `~/.ws-agent/ws-agent.json` 的 `logging` 下。
...
- WsAgent 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `ws-agent plugins enable diagnostics-otel` 启用插件。
...
- `ws-agent.tokens`（计数器，属性：`ws-agent.token`、`ws-agent.channel`、`ws-agent.provider`、`ws-agent.model`）
- `ws-agent.cost.usd`（计数器，属性：`ws-agent.channel`、`ws-agent.provider`、`ws-agent.model`）
- `ws-agent.run.duration_ms`（直方图，属性：`ws-agent.channel`、`ws-agent.provider`、`ws-agent.model`）
- `ws-agent.context.tokens`（直方图，属性：`ws-agent.context`、`ws-agent.channel`、`ws-agent.provider`、`ws-agent.model`）

消息流：

- `ws-agent.webhook.received`（计数器，属性：`ws-agent.channel`、`ws-agent.webhook`）
- `ws-agent.webhook.error`（计数器，属性：`ws-agent.channel`、`ws-agent.webhook`）
- `ws-agent.webhook.duration_ms`（直方图，属性：`ws-agent.channel`、`ws-agent.webhook`）
- `ws-agent.message.queued`（计数器，属性：`ws-agent.channel`、`ws-agent.source`）
- `ws-agent.message.processed`（计数器，属性：`ws-agent.channel`、`ws-agent.outcome`）
- `ws-agent.message.duration_ms`（直方图，属性：`ws-agent.channel`、`ws-agent.outcome`）

队列 + 会话：

- `ws-agent.queue.lane.enqueue`（计数器，属性：`ws-agent.lane`）
- `ws-agent.queue.lane.dequeue`（计数器，属性：`ws-agent.lane`）
- `ws-agent.queue.depth`（直方图，属性：`ws-agent.lane` 或 `ws-agent.channel=heartbeat`）
- `ws-agent.queue.wait_ms`（直方图，属性：`ws-agent.lane`）
- `ws-agent.session.state`（计数器，属性：`ws-agent.state`、`ws-agent.reason`）
- `ws-agent.session.stuck`（计数器，属性：`ws-agent.state`）
- `ws-agent.session.stuck_age_ms`（直方图，属性：`ws-agent.state`）
- `ws-agent.run.attempt`（计数器，属性：`ws-agent.attempt`）
...
- `ws-agent.webhook.error`
  - `ws-agent.channel`、`ws-agent.webhook`、`ws-agent.chatId`、`ws-agent.error`
- `ws-agent.message.processed`
  - `ws-agent.channel`、`ws-agent.outcome`、`ws-agent.chatId`、`ws-agent.messageId`、`ws-agent.sessionKey`、`ws-agent.sessionId`、`ws-agent.reason`
- `ws-agent.session.stuck`
  - `ws-agent.state`、`ws-agent.ageMs`、`ws-agent.queueDepth`、`ws-agent.sessionKey`、`ws-agent.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `ws-agent doctor`。
