...
WsAgent features that can generate provider usage or paid API calls.
...
- `ws-agent status --usage` and `ws-agent channels list` show provider **usage windows**
...
WsAgent can pick up credentials from:
...
- `ws-agent status --usage`
- `ws-agent models status --json`
...
`ws-agent models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
