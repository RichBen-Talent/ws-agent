---
summary: "How WsAgent builds prompt context and reports token usage + costs"
read_when:
...
WsAgent tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

WsAgent assembles its own system prompt on every run. It includes:
...
For images, WsAgent downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from WsAgent session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `ws-agent status --usage` and `ws-agent channels list` show
...
`cacheWrite`. If pricing is missing, WsAgent shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. WsAgent can
...
Anthropic's 1M context window is currently beta-gated. WsAgent can inject the
