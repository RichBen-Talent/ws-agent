...
This document explains how WsAgent manages sessions end-to-end:
...
WsAgent is designed around a single **Gateway process** that owns session state.
...
WsAgent persists sessions in two layers:
...
- Transcripts: `~/.ws-agent/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

WsAgent resolves these via `src/config/sessions.ts`.
...
WsAgent intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (WsAgent consumes the events, but Pi decides when to compact).
...
WsAgent also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, WsAgent bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, WsAgent leaves it alone.
...
- `ws-agent status` (CLI)
- `ws-agent sessions` / `sessions --json`
...
WsAgent supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- WsAgent strips/suppresses this in the delivery layer.

As of `2026.1.10`, WsAgent also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
WsAgent uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but WsAgent’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `ws-agent status`.
