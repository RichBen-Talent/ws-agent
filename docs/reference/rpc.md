...
WsAgent integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- WsAgent owns lifecycle when `channels.signal.autoStart=true`.
...
- WsAgent spawns `imsg rpc` as a child process (legacy iMessage integration).
