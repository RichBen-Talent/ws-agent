...
- `/usage cost` in chats: local cost summary aggregated from WsAgent session logs.
- CLI: `ws-agent status --usage` prints a full per-provider breakdown.
- CLI: `ws-agent channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
