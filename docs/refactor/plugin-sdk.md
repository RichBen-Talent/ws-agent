...
- Publish as `ws-agent/plugin-sdk` (or export from core under `ws-agent/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `WsAgentPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: WsAgentConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: WsAgentConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: WsAgentConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: WsAgentConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: WsAgentConfig): string;
  };
};
```
...
- Introduce `ws-agent/plugin-sdk`.
- Add `api.runtime` to `WsAgentPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `wsAgentRuntime: ">=2026.2.0"`).
