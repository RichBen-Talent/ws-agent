...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.ws-agent/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.ws-agent/node.json` (node id + pairing token).
...
Path: `~/.ws-agent/exec-approvals.json`
...
- Unix socket at `~/.ws-agent/exec-approvals.sock` (0600).
