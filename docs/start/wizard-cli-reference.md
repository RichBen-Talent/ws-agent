...
read_when:
  - You need detailed behavior for ws-agent onboard
...
This page is the full reference for `ws-agent onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.ws-agent/ws-agent.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `ws-agent doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `ws-agent pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `ws-agent health`.
    - `ws-agent status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.ws-agent/.env` so launchd can read it.
...
- OAuth credentials: `~/.ws-agent/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.ws-agent/agents/<agentId>/agent/auth-profiles.json`

<Note>
Headless and server tip: complete OAuth on a machine with a browser, then copy
`~/.ws-agent/credentials/oauth.json` (or `$WS_AGENT_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`ws-agent onboard`](/cli/onboard)
