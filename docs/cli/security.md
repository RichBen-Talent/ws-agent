---
summary: "CLI reference for `ws-agent security` (audit and fix common security footguns)"
read_when:
...
It also warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `ws-agent.browserConfigEpoch`) and recommends `ws-agent sandbox recreate --browser --all`.
...
```bash
ws-agent security audit --json | jq '.summary'
ws-agent security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
ws-agent security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
