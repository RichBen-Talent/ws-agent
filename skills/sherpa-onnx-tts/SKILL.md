...
1. Download the runtime for your OS (extracts into `~/.ws-agent/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `~/.ws-agent/tools/sherpa-onnx-tts/models`)

Update `~/.ws-agent/ws-agent.json`:
...
```json5
{
  skills: {
    entries: {
      "sherpa-onnx-tts": {
        env: {
          SHERPA_ONNX_RUNTIME_DIR: "~/.ws-agent/tools/sherpa-onnx-tts/runtime",
          SHERPA_ONNX_MODEL_DIR: "~/.ws-agent/tools/sherpa-onnx-tts/models/vits-piper-en_US-lessac-high",
        },
      },
    },
  },
