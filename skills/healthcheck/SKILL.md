---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for WsAgent deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, WsAgent cron scheduling for periodic checks, or version status checks on a machine running WsAgent (laptop, workstation, Pi, VPS).
---

# WsAgent Host Hardening

## Overview

Assess and harden the host running WsAgent, then align it to a user-defined risk tolerance without breaking access. Use WsAgent security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim WsAgent changes the host firewall, SSH, or OS updates; it does not.
...
5. WsAgent gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if WsAgent can access sensitive data.
...
### 2) Run WsAgent security audits (read-only)

As part of the default read-only checks, run `ws-agent security audit --deep`. Only offer alternatives if the user requests them:

1. `ws-agent security audit` (faster, non-probing)
2. `ws-agent security audit --json` (structured output)

Offer to apply WsAgent safe defaults (numbered):

1. `ws-agent security audit --fix`

Be explicit that `--fix` only tightens WsAgent defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check WsAgent version/update status (read-only)

As part of the default read-only checks, run `ws-agent update status`.
...
- Credential hygiene notes (location of WsAgent creds, prefer disk encryption)
...
After WsAgent install or first hardening pass, run at least one baseline audit and version check:

- `ws-agent security audit`
- `ws-agent security audit --deep`
- `ws-agent update status`

Ongoing monitoring is recommended. Use the WsAgent cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `ws-agent cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `ws-agent update status`
...
Before creating, `ws-agent cron list` and match on exact `name`. If found, `ws-agent cron edit <id> ...`.
If not found, `ws-agent cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `ws-agent update status` (preferred for source checkouts and channels)
2. `npm view ws-agent version` (published npm version)

## WsAgent command accuracy

Use only supported commands and flags:

- `ws-agent security audit [--deep] [--fix] [--json]`
- `ws-agent status` / `ws-agent status --deep`
- `ws-agent health --json`
- `ws-agent update status`
- `ws-agent cron add|list|runs|run`

Do not invent CLI flags or imply WsAgent enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by WsAgent compaction:
