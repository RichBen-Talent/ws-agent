...
@MainActor
struct WsAgentChatComposer: View {
    @Bindable var viewModel: WsAgentChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(WsAgentChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(WsAgentChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \WsAgentPendingAttachment.id)
                { (att: WsAgentPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            WsAgentPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(WsAgentChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(WsAgentChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(WsAgentChatTheme.subtleCard)
        .clipShape(Capsule())
    }
