...
private let chatUILogger = Logger(subsystem: "ai.ws-agent", category: "WsAgentChatUI")

@MainActor
@Observable
public final class WsAgentChatViewModel {
    public private(set) var messages: [WsAgentChatMessage] = []
    ...
    public var attachments: [WsAgentPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [WsAgentChatPendingToolCall] = []
    public private(set) var sessions: [WsAgentChatSessionEntry] = []
    private let transport: any WsAgentChatTransport
    ...
    private var pendingToolCallsById: [String: WsAgentChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any WsAgentChatTransport) {
    ...
    public var sessionChoices: [WsAgentChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [WsAgentChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: WsAgentPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [WsAgentChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: WsAgentChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: WsAgentChatMessage) -> WsAgentChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> WsAgentChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return WsAgentChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: WsAgentChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [WsAgentChatMessage],
        incoming: [WsAgentChatMessage]) -> [WsAgentChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [WsAgentChatMessage]) -> [WsAgentChatMessage] {
        var result: [WsAgentChatMessage] = []
    ...
    private static func dedupeKey(for message: WsAgentChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [WsAgentChatMessageContent] = [
            WsAgentChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> WsAgentChatAttachmentPayload in
            WsAgentChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> WsAgentChatSessionEntry {
        WsAgentChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: WsAgentChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: WsAgentChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: WsAgentAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = WsAgentChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> WsAgentPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
