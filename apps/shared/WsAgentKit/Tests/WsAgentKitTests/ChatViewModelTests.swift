...
private final class TestChatTransport: @unchecked Sendable, WsAgentChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [WsAgentChatHistoryPayload]
    private let sessionsResponses: [WsAgentChatSessionsListResponse]

    private let stream: AsyncStream<WsAgentChatTransportEvent>
    private let continuation: AsyncStream<WsAgentChatTransportEvent>.Continuation

    init(
        historyResponses: [WsAgentChatHistoryPayload],
        sessionsResponses: [WsAgentChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<WsAgentChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> WsAgentChatHistoryPayload {
        ...
        return self.historyResponses.last ?? WsAgentChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [WsAgentChatAttachmentPayload]) async throws -> WsAgentChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return WsAgentChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> WsAgentChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? WsAgentChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: WsAgentChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = WsAgentChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = WsAgentChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { WsAgentChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = WsAgentChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = WsAgentChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = WsAgentChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { WsAgentChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
