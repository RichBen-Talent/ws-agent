...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.wsagent.mac`) even if the shipped
    ...
    {
        if environment["WS_AGENT_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "ws-agent.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "ws-agent.nixMode") { return true }
