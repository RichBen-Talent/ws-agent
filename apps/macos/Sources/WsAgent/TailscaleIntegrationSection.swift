...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.ws-agent/ws-agent.json. Prefer WS_AGENT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.ws-agent/ws-agent.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.ws-agent/ws-agent.json. Restart the gateway to apply."
        }
