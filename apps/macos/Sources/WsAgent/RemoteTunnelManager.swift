...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.ws-agent", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous WsAgent run already has an SSH listener on the expected port (common after restarts),
