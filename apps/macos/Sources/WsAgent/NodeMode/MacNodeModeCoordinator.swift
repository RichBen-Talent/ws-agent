...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.ws-agent", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: WsAgentNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [WsAgentCapability.canvas.rawValue, WsAgentCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(WsAgentCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if WsAgentLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(WsAgentCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            WsAgentSystemCommand.which.rawValue,
            WsAgentSystemCommand.run.rawValue,
            WsAgentSystemCommand.execApprovalsGet.rawValue,
            WsAgentSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(WsAgentCapability.camera.rawValue) {
        ...
        if capsSet.contains(WsAgentCapability.location.rawValue) {
