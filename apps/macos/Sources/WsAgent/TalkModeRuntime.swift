...
actor TalkModeRuntime {
    ...
    private let logger = Logger(subsystem: "ai.ws-agent", category: "talk.runtime")
    private let ttsLogger = Logger(subsystem: "ai.ws-agent", category: "talk.tts")
    ...
    private func latestAssistantText(sessionKey: String, since: Double? = nil) async -> String? {
        do {
            ...
            let decoded: [WsAgentChatMessage] = messages.compactMap { item in
