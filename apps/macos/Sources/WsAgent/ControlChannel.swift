...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: WsAgentProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.ws-agent", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: WsAgentKit.AnyCodable]()) {
                $0[$1.key] = WsAgentKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: WsAgentProtocol.AnyCodable?) -> [String: WsAgentProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: WsAgentProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: WsAgentKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: WsAgentProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("ws-agent.control.heartbeat")
    static let controlAgentEvent = Notification.Name("ws-agent.control.agent")
}
