...
enum DebugActions {
    private static let verboseDefaultsKey = "ws-agent.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "ws-agent.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = WsAgentPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "WsAgent", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = WsAgentPaths.configURL
        guard
