...
enum WsAgentLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = WsAgentOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = WsAgentFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        WsAgentLogging.bootstrapIfNeeded()
        let label = WsAgentLogging.makeLabel(subsystem: subsystem, category: category)
...
struct WsAgentOSLogHandler: LogHandler {
...
struct WsAgentFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = WsAgentLogging.parseLabel(self.label)
