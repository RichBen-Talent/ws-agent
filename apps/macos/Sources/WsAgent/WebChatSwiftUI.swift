...
private let webChatSwiftLogger = Logger(subsystem: "ai.ws-agent", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: WsAgentChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> WsAgentChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(WsAgentChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> WsAgentChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                WsAgentGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<WsAgentChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any WsAgentChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = WsAgentChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: WsAgentChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<WsAgentChatView>) -> NSViewController
    {
