...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.ws-agent", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> WsAgentSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: WsAgentSessionPreviewEntry,
    ...
    private static func cache(payload: WsAgentSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: WsAgentChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [WsAgentKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [WsAgentKit.AnyCodable]) -> [WsAgentChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(WsAgentChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: WsAgentChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: WsAgentChatMessage) -> Bool {
    ...
    private static func toolNames(for message: WsAgentChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: WsAgentChatMessage) -> String? {
