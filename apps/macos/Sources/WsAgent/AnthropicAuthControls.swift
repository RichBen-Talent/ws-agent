...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: WsAgentOAuthStore.AnthropicOAuthStatus = WsAgentOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: WsAgentOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = WsAgentOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = WsAgentOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try WsAgentOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. WsAgent can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: WsAgentOAuthStore.AnthropicOAuthStatus,
