...
enum CommandResolver {
    private static let projectRootDefaultsKey = "ws-agent.gatewayProjectRootPath"
    private static let helperName = "ws-agent"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let wsAgentEntry = root.appendingPathComponent("ws-agent.mjs").path
        if FileManager().isReadableFile(atPath: wsAgentEntry) { return wsAgentEntry }
        let binEntry = root.appendingPathComponent("bin/ws-agent.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/ws-agent")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let wsAgentPaths = self.wsAgentManagedPaths(home: home)
        if !wsAgentPaths.isEmpty {
            extras.insert(contentsOf: wsAgentPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + wsAgentPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func wsAgentManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".ws-agent"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func wsAgentExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectWsAgentExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("ws-agent.mjs").path,
            root.appendingPathComponent("bin/ws-agent.js").path,
        ]
    ...
    static func hasAnyWsAgentInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.wsAgentExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func wsAgentNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let wsAgentPath = self.projectWsAgentExecutable(projectRoot: root) {
                return [wsAgentPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "ws-agent", subcommand] + extraArgs
            }
            if let wsAgentPath = self.wsAgentExecutable(searchPaths: searchPaths) {
                return [wsAgentPath, subcommand] + extraArgs
            }

            let missingEntry = """
            ws-agent entrypoint missing (looked for dist/index.js or ws-agent.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real ws-agent CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/ws-agent"
        ...
        \(cliSection)
        \(projectSection)
        if command -v ws-agent >/dev/null 2>&1; then
          CLI="$(command -v ws-agent)"
          ws-agent \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/ws-agent.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/ws-agent.mjs"
            node "$PRJ/ws-agent.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/ws-agent.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/ws-agent.js"
            node "$PRJ/bin/ws-agent.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent ws-agent"
          pnpm --silent ws-agent \(quotedArgs);
        else
          echo "ws-agent CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? WsAgentConfigFile.loadDict()
