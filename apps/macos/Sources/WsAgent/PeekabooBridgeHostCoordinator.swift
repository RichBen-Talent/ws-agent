...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.ws-agent", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: WsAgentPeekabooBridgeServices?
    private static var wsAgentSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("WsAgent", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = WsAgentPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.wsAgentSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.wsAgentSocketPath, privacy: .public)")
    }
...
@MainActor
private final class WsAgentPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.wsagent.peekaboo")
