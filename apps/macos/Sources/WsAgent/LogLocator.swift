...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["WS_AGENT_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/ws-agent")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("ws-agent-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("ws-agent-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/ws-agent/ (rolling or stdout), or nil if none exist.
