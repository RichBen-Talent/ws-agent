...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: WsAgentProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: WsAgentProtocol.AnyCodable] = [
            "presence": .init([WsAgentProtocol.AnyCodable(entry)]),
        ]
        let payload = WsAgentProtocol.AnyCodable(payloadMap)
