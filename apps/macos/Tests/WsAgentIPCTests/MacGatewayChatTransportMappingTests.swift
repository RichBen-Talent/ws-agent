...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: WsAgentProtocol.AnyCodable(["ok": WsAgentProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: WsAgentProtocol.AnyCodable(["ok": WsAgentProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = WsAgentProtocol.AnyCodable([
            "runId": WsAgentProtocol.AnyCodable("run-1"),
            "sessionKey": WsAgentProtocol.AnyCodable("main"),
            "state": WsAgentProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: WsAgentProtocol.AnyCodable(["a": WsAgentProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
