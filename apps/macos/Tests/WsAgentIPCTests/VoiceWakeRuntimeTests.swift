...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["ws-agent"]
        let text = "hey ws-agent"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey ws-agent do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["ws-agent"], minPostTriggerGap: 0.3)
