...
@MainActor
final class WsAgentAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.wsagent.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.wsagent.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.wsagent.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "ws-agent.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "ws-agent.watch.promptId"
    static let sessionKeyKey = "ws-agent.watch.sessionKey"
    static let actionPrimaryIDKey = "ws-agent.watch.action.primary.id"
    static let actionPrimaryLabelKey = "ws-agent.watch.action.primary.label"
    static let actionSecondaryIDKey = "ws-agent.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "ws-agent.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "ws-agent.watch.action.primary"
    static let actionSecondaryIdentifier = "ws-agent.watch.action.secondary"
    static let categoryPrefix = "ws-agent.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: WsAgentWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> WsAgentWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return WsAgentWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "WsAgent" : title
        content.body = body
    ...
    private static func categoryActions(
        primaryAction: WsAgentWatchAction,
        secondaryAction: WsAgentWatchAction?) -> [UNNotificationAction]
    {
...
@main
struct WsAgentApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(WsAgentAppDelegate.self) private var appDelegate
...
extension WsAgentApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("WsAgent: installing uncaught exception handler")
