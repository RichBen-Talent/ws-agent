...
struct IOSGatewayChatTransport: WsAgentChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.ws-agent", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> WsAgentChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(WsAgentChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> WsAgentChatHistoryPayload {
        ...
        return try JSONDecoder().decode(WsAgentChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [WsAgentChatAttachmentPayload]) async throws -> WsAgentChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [WsAgentChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(WsAgentChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(WsAgentGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: WsAgentGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: WsAgentChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: WsAgentAgentEventPayload.self)
                        {
