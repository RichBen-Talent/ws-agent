...
enum GatewaySettingsStore {
    private static let gatewayService = "ai.wsagent.gateway"
    private static let nodeService = "ai.wsagent.node"
    private static let talkService = "ai.wsagent.talk"
...
enum GatewayDiagnostics {
    private static let logger = Logger(subsystem: "ai.wsagent.ios", category: "GatewayDiag")
    private static let queue = DispatchQueue(label: "ai.wsagent.gateway.diagnostics")
    ...
    private static var fileURL: URL? {
        FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first?
            .appendingPathComponent("ws-agent-gateway.log")
    }
