...
final class RemindersService: RemindersServicing {
    func list(params: WsAgentRemindersListParams) async throws -> WsAgentRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[WsAgentReminderPayload], Error>) in
        ...
        return WsAgentRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: WsAgentRemindersAddParams) async throws -> WsAgentRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = WsAgentReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return WsAgentRemindersAddPayload(reminder: payload)
    }
