...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__WS_AGENT_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<WsAgentTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = WsAgentTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> WsAgentTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return WsAgentTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> WsAgentTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> WsAgentTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return WsAgentTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> WsAgentTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = WsAgentTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: WsAgentTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: WsAgentAgentEventPayload.self) else {
                continue
            }
