...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> WsAgentNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> WsAgentNetworkStatusPayload {
        let status: WsAgentNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> WsAgentNetworkStatusPayload {
        WsAgentNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
