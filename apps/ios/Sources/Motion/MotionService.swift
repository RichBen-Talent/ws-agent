...
final class MotionService: MotionServicing {
    func activities(params: WsAgentMotionActivityParams) async throws -> WsAgentMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[WsAgentMotionActivityEntry], Error>) in
        ...
        return WsAgentMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: WsAgentPedometerParams) async throws -> WsAgentPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<WsAgentPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = WsAgentPedometerPayload(
