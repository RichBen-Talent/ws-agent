...
final class ContactsService: ContactsServicing {
    ...
    func search(params: WsAgentContactsSearchParams) async throws -> WsAgentContactsSearchPayload {
        ...
        return WsAgentContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: WsAgentContactsAddParams) async throws -> WsAgentContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return WsAgentContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return WsAgentContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> WsAgentContactPayload {
        WsAgentContactPayload(
