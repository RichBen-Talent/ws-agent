...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.wsagent.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to WsAgent", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to WsAgent gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to WsAgent.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "WsAgentShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "WsAgent is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "WsAgent Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("ws-agent-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
