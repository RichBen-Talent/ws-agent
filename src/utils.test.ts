...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("ws-agent-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("ws-agent-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("ws-agent-lid-a-", (first) => {
      withTempDirSync("ws-agent-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.ws-agent when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "ws-agent-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers WS_AGENT_HOME over HOME", () => {
    vi.stubEnv("WS_AGENT_HOME", "/srv/ws-agent-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/ws-agent-home"));
...
describe("shortenHomePath", () => {
  it("uses $WS_AGENT_HOME prefix when WS_AGENT_HOME is set", () => {
    vi.stubEnv("WS_AGENT_HOME", "/srv/ws-agent-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/ws-agent-home")}/.ws-agent/ws-agent.json`)).toBe(
      "$WS_AGENT_HOME/.ws-agent/ws-agent.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $WS_AGENT_HOME replacement when WS_AGENT_HOME is set", () => {
    vi.stubEnv("WS_AGENT_HOME", "/srv/ws-agent-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/ws-agent-home")}/.ws-agent/ws-agent.json`),
    ).toBe("config: $WS_AGENT_HOME/.ws-agent/ws-agent.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/ws-agent")).toBe(path.resolve(os.homedir(), "ws-agent"));
  });
  ...
  it("prefers WS_AGENT_HOME for tilde expansion", () => {
    vi.stubEnv("WS_AGENT_HOME", "/srv/ws-agent-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/ws-agent")).toBe(path.resolve("/srv/ws-agent-home", "ws-agent"));
