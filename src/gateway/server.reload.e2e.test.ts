...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.WS_AGENT_SKIP_CHANNELS;
    prevSkipGmail = process.env.WS_AGENT_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.WS_AGENT_SKIP_PROVIDERS;
    process.env.WS_AGENT_SKIP_CHANNELS = "0";
    delete process.env.WS_AGENT_SKIP_GMAIL_WATCHER;
    delete process.env.WS_AGENT_SKIP_PROVIDERS;
  });
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.WS_AGENT_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.WS_AGENT_SKIP_GMAIL_WATCHER;
    } else {
      process.env.WS_AGENT_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.WS_AGENT_SKIP_PROVIDERS;
    } else {
      process.env.WS_AGENT_SKIP_PROVIDERS = prevSkipProviders;
    }
  });
