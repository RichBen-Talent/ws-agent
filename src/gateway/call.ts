...
export function buildGatewayConnectionDetails(
  options: { config?: WsAgentConfig; url?: string; configPath?: string } = {},
...
function resolveGatewayCredentials(context: ResolvedGatewayCallContext): {
  ...
  const token =
    ...
    (!context.urlOverride
      ? context.isRemoteMode
        ...
        : trimToUndefined(process.env.WS_AGENT_GATEWAY_TOKEN) ||
          trimToUndefined(process.env.WS_AGENT_GATEWAY_TOKEN) ||
  ...
  const password =
    ...
    (!context.urlOverride
      ? trimToUndefined(process.env.WS_AGENT_GATEWAY_PASSWORD) ||
        trimToUndefined(process.env.WS_AGENT_GATEWAY_PASSWORD) ||
        (context.isRemoteMode
