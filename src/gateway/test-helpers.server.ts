...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "WS_AGENT_BUNDLED_PLUGINS_DIR",
  "WS_AGENT_SKIP_CHANNELS",
  "WS_AGENT_SKIP_PROVIDERS",
  "WS_AGENT_SKIP_CRON",
  "WS_AGENT_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "ws-agent-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.WS_AGENT_SKIP_PROVIDERS = "1";
  process.env.WS_AGENT_SKIP_CRON = "1";
  process.env.WS_AGENT_TEST_MINIMAL_GATEWAY = "1";
  process.env.WS_AGENT_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "ws-agent-test-no-bundled-extensions")
    : "ws-agent-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "ws-agent-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".ws-agent-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__wsAgentTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__wsAgentTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["WS_AGENT_GATEWAY_TOKEN"]);
  const prev = process.env.WS_AGENT_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.WS_AGENT_GATEWAY_TOKEN = fallbackToken;
  }
