...
const { createWsAgentTools } = await import("../agents/ws-agent-tools.js");
...
beforeAll(async () => {
  envSnapshot = captureEnv(["WS_AGENT_GATEWAY_PORT", "WS_AGENT_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.WS_AGENT_GATEWAY_PORT = String(gatewayPort);
  process.env.WS_AGENT_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createWsAgentTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
...
describe("sessions_send label lookup", () => {
  it("finds session by label and sends message", { timeout: 60_000 }, async () => {
    ...
    if (!configPath) {
      throw new Error("WS_AGENT_CONFIG_PATH missing in gateway test environment");
    }
