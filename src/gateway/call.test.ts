...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("throws for insecure ws:// remote URLs (CWE-319)", () => {
    ...
    expect((thrown as Error).message).toContain("ws-agent doctor --fix");
  });
...
describe("callGateway password resolution", () => {
  ...
  const explicitAuthCases = [
    {
      label: "password",
      authKey: "password",
      envKey: "WS_AGENT_GATEWAY_PASSWORD",
      envValue: "from-env",
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["WS_AGENT_GATEWAY_PASSWORD", "WS_AGENT_GATEWAY_TOKEN"]);
    resetGatewayCallMocks();
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.WS_AGENT_GATEWAY_PASSWORD = envPassword;
    }
