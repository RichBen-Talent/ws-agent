...
beforeAll(async () => {
  previousToken = process.env.WS_AGENT_GATEWAY_TOKEN;
  process.env.WS_AGENT_GATEWAY_TOKEN = "test-gateway-token-1234567890";
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env.WS_AGENT_GATEWAY_TOKEN = previousToken;
  }
});
...
test.each([
  { clientId: "ws-agent-ios", platform: "ios" },
  { clientId: "ws-agent-android", platform: "android" },
