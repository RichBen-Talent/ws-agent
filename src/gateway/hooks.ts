...
export function resolveHooksConfig(cfg: WsAgentConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: WsAgentConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-ws-agent-token"] === "string"
      ? req.headers["x-ws-agent-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
