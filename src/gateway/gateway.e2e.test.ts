...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "ws-agent-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "ws-agent");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.ws-agent-tool-probe.${nonceA}.txt`);
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "ws-agent-wizard-home-"));
      process.env.HOME = tempHome;
