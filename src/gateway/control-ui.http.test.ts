...
describe("handleControlUiHttpRequest", () => {
  async function withControlUiRoot<T>(params: {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "ws-agent-ui-"));
    try {
  ...
  async function withBasePathRootFixture<T>(params: {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "ws-agent-ui-root-"));
    try {
  ...
  it("serves bootstrap config JSON under basePath", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const { res, end } = makeMockHttpResponse();
        const handled = handleControlUiHttpRequest(
          { url: `/ws-agent${CONTROL_UI_BOOTSTRAP_CONFIG_PATH}`, method: "GET" } as IncomingMessage,
          res,
          {
            basePath: "/ws-agent",
            root: { kind: "resolved", path: tmp },
        ...
        expect(handled).toBe(true);
        const parsed = parseBootstrapPayload(end);
        expect(parsed.basePath).toBe("/ws-agent");
        expect(parsed.assistantName).toBe("Ops");
        expect(parsed.assistantAvatar).toBe("/ws-agent/avatar/main");
        expect(parsed.assistantAgentId).toBe("main");
      },
    });
  });
  ...
  it("rejects symlinked assets that resolve outside control-ui root", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        ...
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "ws-agent-ui-outside-"));
        try {
  ...
  it("rejects symlinked SPA fallback index.html outside control-ui root", async () => {
    await withControlUiRoot({
      fn: async (tmp) => {
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "ws-agent-ui-index-outside-"));
        try {
  ...
  it("rejects absolute-path escape attempts under basePath routes", async () => {
    await withBasePathRootFixture({
      ...
      fn: async ({ root, sibling }) => {
        ...
        const handled = handleControlUiHttpRequest(
          { url: `/ws-agent/${absolutePathUrl}`, method: "GET" } as IncomingMessage,
          res,
          {
            basePath: "/ws-agent",
  ...
  it("rejects symlink escape attempts under basePath routes", async () => {
    await withBasePathRootFixture({
      ...
      fn: async ({ root, sibling }) => {
        ...
        const { res, end } = makeMockHttpResponse();
        const handled = handleControlUiHttpRequest(
          { url: "/ws-agent/assets/leak.txt", method: "GET" } as IncomingMessage,
          res,
          {
            basePath: "/ws-agent",
            root: { kind: "resolved", path: root },
