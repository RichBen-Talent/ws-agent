...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("ws-agent-fs-safe-");
  ...
  it("blocks traversal outside root", async () => {
    const root = await tempDirs.make("ws-agent-fs-safe-root-");
    const outside = await tempDirs.make("ws-agent-fs-safe-outside-");
