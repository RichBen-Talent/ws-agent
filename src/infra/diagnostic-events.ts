...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __wsAgentDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__wsAgentDiagnosticEventsState) {
    globalStore.__wsAgentDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__wsAgentDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: WsAgentConfig): boolean {
