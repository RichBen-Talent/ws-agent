...
import { resolveWsAgentPackageRoot, resolveWsAgentPackageRootSync } from "./ws-agent-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveWsAgentPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "ws-agent" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveWsAgentPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // ws-agent.mjs or dist/<bundle>.js
