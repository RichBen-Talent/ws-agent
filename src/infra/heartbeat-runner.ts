...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: WsAgentConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: WsAgentConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: WsAgentConfig, agentId?: string): boolean {
...
function resolveHeartbeatConfig(cfg: WsAgentConfig, agentId?: string): HeartbeatConfig | undefined {
...
function resolveHeartbeatAgents(cfg: WsAgentConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: WsAgentConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: WsAgentConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: WsAgentConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
