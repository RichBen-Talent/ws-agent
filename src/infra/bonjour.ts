...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.WS_AGENT_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "WsAgent";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(WsAgent\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.WS_AGENT_MDNS_HOSTNAME?.trim() ||
    process.env.WS_AGENT_MDNS_HOSTNAME?.trim() ||
    "ws-agent";
  const hostname =
    hostnameRaw
