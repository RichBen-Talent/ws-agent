...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.WS_AGENT_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*ws-agent-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "ws-agent";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "ws-agent");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "ws-agent-gw");
  ...
  records.push(`_ws-agent-gw._tcp IN PTR ${instanceLabel}._ws-agent-gw._tcp`);
  records.push(`${instanceLabel}._ws-agent-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._ws-agent-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; ws-agent-content-hash: ${contentHash}\n${contentBody}`;
}
