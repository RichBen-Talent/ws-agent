...
let ensureWsAgentCliOnPath: typeof import("./path-env.js").ensureWsAgentCliOnPath;

describe("ensureWsAgentCliOnPath", () => {
  const envKeys = [
    "PATH",
    "WS_AGENT_PATH_BOOTSTRAPPED",
    "WS_AGENT_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureWsAgentCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling ws-agent exists", () => {
    const tmp = abs("/tmp/ws-agent-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "ws-agent");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.WS_AGENT_PATH_BOOTSTRAPPED;

    ensureWsAgentCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.WS_AGENT_PATH_BOOTSTRAPPED = "1";
    ensureWsAgentCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/ws-agent-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "ws-agent");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/ws-agent-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "ws-agent");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "ws-agent");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/ws-agent-path/case-linuxbrew");
