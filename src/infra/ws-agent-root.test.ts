...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__wsAgent_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "ws-agent-root");
...
describe("resolveWsAgentPackageRoot", () => {
  let resolveWsAgentPackageRoot: typeof import("./ws-agent-root.js").resolveWsAgentPackageRoot;
  let resolveWsAgentPackageRootSync: typeof import("./ws-agent-root.js").resolveWsAgentPackageRootSync;

  beforeAll(async () => {
    ({ resolveWsAgentPackageRoot, resolveWsAgentPackageRootSync } =
      await import("./ws-agent-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "ws-agent");
    const pkgRoot = path.join(project, "node_modules", "ws-agent");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "ws-agent" }));

    expect(resolveWsAgentPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "ws-agent");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "ws-agent.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "ws-agent" }));

    expect(resolveWsAgentPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveWsAgentPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-ws-agent package roots", async () => {
    const pkgRoot = fx("not-ws-agent");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-ws-agent" }));

    expect(resolveWsAgentPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveWsAgentPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveWsAgentPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
