...
export function triggerWsAgentRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.WS_AGENT_SYSTEMD_UNIT,
        process.env.WS_AGENT_PROFILE,
      );
  ...
  const label =
    process.env.WS_AGENT_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.WS_AGENT_PROFILE);
