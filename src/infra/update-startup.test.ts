...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ws-agent-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["WS_AGENT_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.WS_AGENT_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveWsAgentPackageRoot } = await import("./ws-agent-root.js"));
  ...
  async function runUpdateCheckAndReadState(channel: "stable" | "beta") {
    vi.mocked(resolveWsAgentPackageRoot).mockResolvedValue("/opt/ws-agent");
