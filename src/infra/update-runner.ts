...
const DEFAULT_PACKAGE_NAME = "ws-agent";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ws-agent-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "ws-agent.mjs");
    ...
    const doctorStep = await runStep(
      step("ws-agent doctor", doctorArgv, gitRoot, { WS_AGENT_UPDATE_IN_PROGRESS: "1" }),
    );
