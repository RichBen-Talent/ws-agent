...
vi.mock("./ws-agent-root.js", () => ({
  resolveWsAgentPackageRoot: vi.fn(async () => null),
  resolveWsAgentPackageRootSync: vi.fn(() => null),
}));
...
let wsAgentRoot: typeof import("./ws-agent-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    wsAgentRoot = await import("./ws-agent-root.js");
  });
  ...
  it("uses resolveWsAgentPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/ws-agent");
    (
      wsAgentRoot.resolveWsAgentPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/ws-agent"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "ws-agent" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "ws-agent.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-ws-agent");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/ws-agent-bundle");
    (
      wsAgentRoot.resolveWsAgentPackageRootSync as unknown as ReturnType<typeof vi.fn>
