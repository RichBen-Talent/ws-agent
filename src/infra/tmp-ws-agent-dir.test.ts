...
import { POSIX_WS_AGENT_TMP_DIR, resolvePreferredWsAgentTmpDir } from "./tmp-ws-agent-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredWsAgentTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `ws-agent-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredWsAgentTmpDir({
    accessSync,
...
describe("resolvePreferredWsAgentTmpDir", () => {
  it("prefers /tmp/ws-agent when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_WS_AGENT_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/ws-agent when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_WS_AGENT_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/ws-agent when /tmp/ws-agent is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/ws-agent when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/ws-agent is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/ws-agent is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/ws-agent is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
