...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: WsAgentConfig | null = null;
  ...
  get(key: string, cfg: WsAgentConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: WsAgentConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: WsAgentConfig): void {
