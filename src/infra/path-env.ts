...
function candidateBinDirs(opts: EnsureWsAgentPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `ws-agent` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "ws-agent");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.WS_AGENT_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "ws-agent"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `ws-agent` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureWsAgentCliOnPath(opts: EnsureWsAgentPathOpts = {}) {
  if (isTruthyEnvValue(process.env.WS_AGENT_PATH_BOOTSTRAPPED)) {
    return;
  }
