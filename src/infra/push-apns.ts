...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.WS_AGENT_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.WS_AGENT_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set WS_AGENT_APNS_TEAM_ID and WS_AGENT_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.WS_AGENT_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.WS_AGENT_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.WS_AGENT_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set WS_AGENT_APNS_PRIVATE_KEY_P8 or WS_AGENT_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading WS_AGENT_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createWsAgentPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    "ws-agent": createWsAgentPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
