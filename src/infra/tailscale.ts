...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.WS_AGENT_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for WsAgent. You can keep running the web gateway without it: \`${formatCliCommand("ws-agent gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
