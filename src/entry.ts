...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "ws-agent.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "ws-agent.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.WS_AGENT_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.WS_AGENT_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.WS_AGENT_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        "[ws-agent] Failed to respawn CLI:",
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[ws-agent] ${parsed.error}`);
      process.exit(2);
    }
