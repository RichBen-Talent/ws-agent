...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-ws-agent-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-ws-agent-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-ws-agent-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the WsAgent gateway (WsAgent.app menubar, or \`${formatCliCommand("ws-agent gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the WsAgent browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the WsAgent browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
