...
vi.mock("./chrome.js", () => ({
  resolveWsAgentUserDataDir: vi.fn(() => "/tmp/ws-agent-test/ws-agent/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveWsAgentUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "ws-agent-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveWsAgentUserDataDir).mockReturnValue(userDataDir);
