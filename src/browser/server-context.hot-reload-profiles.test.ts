...
describe("server-context hot-reload profiles", () => {
  ...
  it("forProfile hot-reloads newly added profiles from config", async () => {
    // Start with only ws-agent profile
    ...
    // 2. Simulate adding a new profile to config (like user editing ws-agent.json)
  ...
  it("forProfile refreshes existing profile config after loadConfig cache updates", async () => {
    ...
    cfgProfiles["ws-agent"] = { cdpPort: 19999, color: "#FF4500" };
    cachedConfig = null;
    ...
    expect(state.resolved.profiles["ws-agent"]?.cdpPort).toBe(19999);
  });
