import type { BrowserConfig, BrowserProfileConfig, WsAgentConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_WS_AGENT_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "ws-agent" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_WS_AGENT_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "ws-agent" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_WS_AGENT_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_WS_AGENT_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an WsAgent browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for ws-agent/ws-agent CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: WsAgentConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_WS_AGENT_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "ws-agent";
