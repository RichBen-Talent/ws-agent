...
import {
  probeAuthenticatedWsAgentRelay,
...
describe("extension-relay-auth", () => {
  ...
  it("accepts authenticated ws-agent relay probe responses", async () => {
    ...
    await withRelayServer(
      ...
      async ({ port }) => {
        ...
        const ok = await probeAuthenticatedWsAgentRelay({
          baseUrl: `http://127.0.0.1:${port}`,
          relayAuthHeader: "x-ws-agent-relay-token",
