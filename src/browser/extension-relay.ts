...
const RELAY_AUTH_HEADER = "x-ws-agent-relay-token";
...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/WsAgent-Extension-Relay",
          revision: "0",
          userAgent: "WsAgent-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload: Record<string, unknown> = {
        Browser: "WsAgent/extension-relay",
  ...
  try {
    ...
    if (
      ...
      (await probeAuthenticatedWsAgentRelay({
