...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredWsAgentTmpDir: vi.fn(() => "/tmp/ws-agent"),
}));
vi.mock("../infra/tmp-ws-agent-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredWsAgentTmpDir.mockReturnValue("/tmp/ws-agent");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredWsAgentTmpDir.mockReturnValue("/tmp/ws-agent-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "ws-agent-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredWsAgentTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "ws-agent-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "ws-agent-preferred", "downloads")}${path.sep}`),
    );
  });
