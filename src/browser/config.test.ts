...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const wsAgent = resolveProfile(resolved, "ws-agent");
    expect(wsAgent?.driver).toBe("ws-agent");
    expect(wsAgent?.cdpPort).toBe(18800);
    expect(wsAgent?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from WS_AGENT_GATEWAY_PORT when unset", () => {
    withEnv({ WS_AGENT_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const wsAgent = resolveProfile(resolved, "ws-agent");
      expect(wsAgent?.cdpPort).toBe(19012);
      expect(wsAgent?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ WS_AGENT_GATEWAY_PORT: undefined }, () => {
      ...
      const wsAgent = resolveProfile(resolved, "ws-agent");
      expect(wsAgent?.cdpPort).toBe(19022);
      expect(wsAgent?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "ws-agent");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        "ws-agent": { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("ws-agent");
  });
