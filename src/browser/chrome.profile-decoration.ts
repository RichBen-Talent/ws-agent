...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".ws-agent-profile-decorated");
}
 ...
 */
export function decorateWsAgentProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_WS_AGENT_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_WS_AGENT_BROWSER_COLOR).toUpperCase();
