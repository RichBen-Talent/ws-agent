...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.WS_AGENT_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/ws-agent" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "ws-agent",
        profiles: {
          "ws-agent": { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchWsAgentChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.WS_AGENT_GATEWAY_PORT;
    process.env.WS_AGENT_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.WS_AGENT_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.WS_AGENT_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayToken === undefined) {
      ...
      process.env.WS_AGENT_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.WS_AGENT_GATEWAY_PASSWORD;
    } else {
      process.env.WS_AGENT_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
