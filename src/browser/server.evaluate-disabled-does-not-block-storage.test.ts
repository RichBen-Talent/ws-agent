  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          "ws-agent": { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.WS_AGENT_GATEWAY_PORT;
    process.env.WS_AGENT_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.WS_AGENT_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.WS_AGENT_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayToken === undefined) {
      ...
      process.env.WS_AGENT_GATEWAY_TOKEN = prevGatewayToken;
    }
    if (prevGatewayPassword === undefined) {
      delete process.env.WS_AGENT_GATEWAY_PASSWORD;
    } else {
      process.env.WS_AGENT_GATEWAY_PASSWORD = prevGatewayPassword;
    }
