...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".ws-agent");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.WS_AGENT_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$WS_AGENT_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via WS_AGENT_STATE_DIR.
