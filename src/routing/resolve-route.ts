...
function listAgents(cfg: WsAgentConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: WsAgentConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: WsAgentConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<WsAgentConfig, EvaluatedBindingsCache>();
