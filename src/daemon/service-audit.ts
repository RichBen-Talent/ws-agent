...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.WS_AGENT_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service WS_AGENT_GATEWAY_TOKEN does not match gateway.auth.token in ws-agent.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `ws-agent gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
