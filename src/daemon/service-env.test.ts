...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.WS_AGENT_GATEWAY_PORT).toBe("18789");
    expect(env.WS_AGENT_GATEWAY_TOKEN).toBe("secret");
    expect(env.WS_AGENT_SERVICE_MARKER).toBe("ws-agent");
    expect(env.WS_AGENT_SERVICE_KIND).toBe("gateway");
    expect(typeof env.WS_AGENT_SERVICE_VERSION).toBe("string");
    expect(env.WS_AGENT_SYSTEMD_UNIT).toBe("ws-agent-gateway.service");
    if (process.platform === "darwin") {
      expect(env.WS_AGENT_LAUNCHD_LABEL).toBe("ai.wsagent.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", WS_AGENT_PROFILE: "work" },
      port: 18789,
    });
    expect(env.WS_AGENT_SYSTEMD_UNIT).toBe("ws-agent-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.WS_AGENT_LAUNCHD_LABEL).toBe("ai.wsagent.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".ws-agent"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", WS_AGENT_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".ws-agent-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", WS_AGENT_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".ws-agent"));
  });

  it("uses WS_AGENT_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", WS_AGENT_STATE_DIR: "/var/lib/ws-agent" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/ws-agent"));
  });

  it("expands ~ in WS_AGENT_STATE_DIR", () => {
    const env = { HOME: "/Users/test", WS_AGENT_STATE_DIR: "~/ws-agent-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/ws-agent-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { WS_AGENT_STATE_DIR: "C:\\State\\ws-agent" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\ws-agent");
  });
});
