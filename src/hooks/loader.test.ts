...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ws-agent-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["WS_AGENT_BUNDLED_HOOKS_DIR"]);
    process.env.WS_AGENT_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
