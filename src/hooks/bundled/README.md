...
This directory contains hooks that ship with WsAgent. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.ws-agent/ws-agent.json`:
...
2. Restart gateway: `pkill -9 -f 'ws-agent.*gateway' && pnpm ws-agent gateway`
3. Enable the hook: `ws-agent hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.ws-agent.ai/automation/hooks
