...
homepage: https://docs.ws-agent.ai/automation/hooks#command-logger
metadata:
  {
    "ws-agent":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.ws-agent/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.ws-agent/logs/commands.log ~/.ws-agent/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/ws-agent`:
   ```
   /home/username/.ws-agent/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
