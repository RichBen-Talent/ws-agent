...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => WsAgentConfig;
  action?: "new" | "reset";
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("ws-agent-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): WsAgentConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
