...
`;
    ...
    expect(parsed["ws-agent"].emoji).toBe("💾");
    expect(parsed["ws-agent"].events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed["ws-agent"].emoji).toBe("📝");
    expect(parsed["ws-agent"].events).toEqual(["command"]);
    expect(parsed["ws-agent"].requires.config).toEqual(["workspace.dir"]);
    expect(parsed["ws-agent"].install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"ws-agent": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"ws-agent": {"events": ["test"]}}');
  });
...
describe("resolveWsAgentMetadata", () => {
  it("extracts ws-agent metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveWsAgentMetadata(frontmatter);
  ...
  it("returns undefined when ws-agent key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        "ws-agent": {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with WsAgent" },
            { id: "npm", kind: "npm", package: "@ws-agent/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@ws-agent/hook");
  });
    ...
    "ws-agent":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with WsAgent" }],
      },
  }
---
...
`;
    ...
    const wsAgent = resolveWsAgentMetadata(frontmatter);
    expect(wsAgent).toBeDefined();
    expect(wsAgent?.emoji).toBe("💾");
    expect(wsAgent?.events).toEqual(["command:new", "command:reset"]);
    expect(wsAgent?.requires?.config).toEqual(["workspace.dir"]);
    expect(wsAgent?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const wsAgent = resolveWsAgentMetadata(frontmatter);
    expect(wsAgent?.emoji).toBe("disk");
    expect(wsAgent?.events).toEqual(["command:new"]);
  });
});
