...
function summarizeGroupPolicy(cfg: WsAgentConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: WsAgentConfig): boolean {
...
function collectModels(cfg: WsAgentConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: WsAgentConfig): Set<string> {
  const baseCfg: WsAgentConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: WsAgentConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: WsAgentConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: WsAgentConfig): boolean {
...
function isBrowserEnabled(cfg: WsAgentConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: WsAgentConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: WsAgentConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep WS_AGENT_STATE_DIR on a local-only volume and re-run "${formatCliCommand("ws-agent security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: WsAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer WS_AGENT_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const wsAgentGatewayToken =
    typeof env.WS_AGENT_GATEWAY_TOKEN === "string" && env.WS_AGENT_GATEWAY_TOKEN.trim()
      ? env.WS_AGENT_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-ws-agent-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: WsAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: WsAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: ws-agent-sandbox-browser), " +
...
export function collectNodeDenyCommandPatternFindings(cfg: WsAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: WsAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: WsAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: WsAgentConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
