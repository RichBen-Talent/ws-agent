...
const resolveStorePath = vi.fn(() => "/tmp/ws-agent-discord-process-test-sessions.json");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/ws-agent-discord-process-test-sessions.json");
  ...
  async function createBlockModeContext() {
    return await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/ws-agent-discord-process-test-sessions.json" },
        channels: {
          discord: {
