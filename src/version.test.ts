...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "ws-agent-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "ws-agent", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "ws-agent", version: "2.3.4" });
  ...
  it("ignores non-ws-agent package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
  ...
  it("prefers WS_AGENT_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        WS_AGENT_VERSION: "9.9.9",
        WS_AGENT_SERVICE_VERSION: "2.2.2",
  ...
  it("uses service and package fallbacks and ignores blank env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        WS_AGENT_VERSION: "   ",
        WS_AGENT_SERVICE_VERSION: "  2.0.0  ",
