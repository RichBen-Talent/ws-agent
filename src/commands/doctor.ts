...
function resolveMode(cfg: WsAgentConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  let cfg: WsAgentConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("ws-agent configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("ws-agent config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("ws-agent setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("ws-agent doctor --fix")}" to apply changes.`);
  }
