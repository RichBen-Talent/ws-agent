...
export async function statusCommand(
  ...
  runtime.log(theme.heading("WsAgent status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("ws-agent security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("ws-agent security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.ws-agent.ai/faq");
  runtime.log("Troubleshooting: https://docs.ws-agent.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("ws-agent status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("ws-agent logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("ws-agent status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("ws-agent gateway probe")}`);
  }
}
