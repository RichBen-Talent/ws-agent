...
import type { WsAgentConfig, GatewayBindMode } from "../config/config.js";
import { resolveGatewayAuth } from "../gateway/auth.js";
import { isLoopbackHost, resolveGatewayBindHost } from "../gateway/net.js";
import { resolveDmAllowState } from "../security/dm-policy-shared.js";
import { note } from "../terminal/note.js";

export async function noteSecurityWarnings(cfg: WsAgentConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("ws-agent security audit --deep")}`;
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.ws-agent.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("ws-agent configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("ws-agent config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("ws-agent doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("ws-agent config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('ws-agent config set session.dmScope "per-channel-peer"') +
