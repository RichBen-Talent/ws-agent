...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveWsAgentAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("ws-agent models auth setup-token")}\` or \`${formatCliCommand("ws-agent configure")}\`.`
          : `Run \`${formatCliCommand("ws-agent configure")}\` or set an API key env var.`;
