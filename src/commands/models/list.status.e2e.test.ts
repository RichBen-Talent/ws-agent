...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveWsAgentAgentDir: vi.fn().mockReturnValue("/tmp/ws-agent-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/ws-agent-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/ws-agent-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveWsAgentAgentDir: mocks.resolveWsAgentAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/ws-agent-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveWsAgentAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/ws-agent-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/ws-agent-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/ws-agent-agent-custom");
