...
export async function loadValidConfigOrThrow(): Promise<WsAgentConfig> {
...
export async function updateConfig(
  mutator: (cfg: WsAgentConfig) => WsAgentConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: WsAgentConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: WsAgentConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("ws-agent agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
