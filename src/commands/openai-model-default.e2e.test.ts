...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: WsAgentConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: WsAgentConfig) => { changed: boolean; next: WsAgentConfig };
  defaultModel: string;
  overrideConfig: WsAgentConfig;
  alreadyDefaultConfig: WsAgentConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: WsAgentConfig) => config,
      applyDefaultConfig: (config: WsAgentConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: WsAgentConfig = { agents: { defaults: {} } };
