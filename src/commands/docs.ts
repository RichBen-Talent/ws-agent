...
const SEARCH_TOOL = "https://docs.ws-agent.ai/mcp.SearchWsAgent";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.ws-agent.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('ws-agent docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.ws-agent.ai/");
      runtime.log(`Search: ${formatCliCommand('ws-agent docs "your query"')}`);
    }
    return;
  }
