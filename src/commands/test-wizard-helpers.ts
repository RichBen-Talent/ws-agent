...
export function requireWsAgentAgentDir(): string {
  const agentDir = process.env.WS_AGENT_AGENT_DIR;
  if (!agentDir) {
    throw new Error("WS_AGENT_AGENT_DIR not set");
  }
  return agentDir;
}
