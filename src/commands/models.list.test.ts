...
const ensureWsAgentModelsJson = vi.fn().mockResolvedValue(undefined);
const ensurePiAuthJsonFromAuthProfiles = vi
  .fn()
  .mockResolvedValue({ wrote: false, authPath: "/tmp/ws-agent-agent/auth.json" });
const resolveWsAgentAgentDir = vi.fn().mockReturnValue("/tmp/ws-agent-agent");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/ws-agent-state",
  loadConfig,
}));

vi.mock("../agents/models-config.js", () => ({
  ensureWsAgentModelsJson,
}));
...
describe("models list/status", () => {
  ...
  it("models list syncs auth-profiles into auth.json before availability checks", async () => {
    ...
    expect(ensurePiAuthJsonFromAuthProfiles).toHaveBeenCalledWith("/tmp/ws-agent-agent");
  });
