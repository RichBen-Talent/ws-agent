...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "WS_AGENT_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("WS_AGENT_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv WS_AGENT_GATEWAY_TOKEN");
    expect(message).not.toContain("WS_AGENT_GATEWAY_PASSWORD");
  });
