...
type EnvSnapshot = {
  HOME?: string;
  WS_AGENT_HOME?: string;
  WS_AGENT_STATE_DIR?: string;
  WS_AGENT_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    WS_AGENT_HOME: process.env.WS_AGENT_HOME,
    WS_AGENT_STATE_DIR: process.env.WS_AGENT_STATE_DIR,
    WS_AGENT_OAUTH_DIR: process.env.WS_AGENT_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: WsAgentConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: WsAgentConfig) {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "ws-agent-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.WS_AGENT_HOME = tempHome;
    process.env.WS_AGENT_STATE_DIR = path.join(tempHome, ".ws-agent");
    delete process.env.WS_AGENT_OAUTH_DIR;
    fs.mkdirSync(process.env.WS_AGENT_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when WS_AGENT_OAUTH_DIR is explicitly configured", async () => {
    process.env.WS_AGENT_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: WsAgentConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
