...
function resolveAgentName(cfg: WsAgentConfig, agentId: string) {
...
function resolveAgentModel(cfg: WsAgentConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: WsAgentConfig): AgentSummary[] {
