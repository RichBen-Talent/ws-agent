...
beforeAll(() => {
  envSnapshot = captureEnv(["WS_AGENT_PROFILE"]);
  process.env.WS_AGENT_PROFILE = "isolated";
});
...
vi.mock("../infra/ws-agent-root.js", () => ({
  resolveWsAgentPackageRoot: vi.fn().mockResolvedValue("/tmp/ws-agent"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/ws-agent/pnpm-lock.yaml",
      markerPath: "/tmp/ws-agent/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("WsAgent status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("ws-agent status --all") ||
          l.includes("ws-agent --profile isolated status --all") ||
          l.includes("ws-agent status --all") ||
          l.includes("ws-agent --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.WS_AGENT_GATEWAY_TOKEN = "abcd1234";
    try {
