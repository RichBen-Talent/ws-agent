...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under WS_AGENT_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("ws-agent-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "ws-agent-oauth-sync-"));
    ...
    process.env.WS_AGENT_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "ws-agent-oauth-nosync-"));
    process.env.WS_AGENT_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside WS_AGENT_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "ws-agent-oauth-external-"));
    process.env.WS_AGENT_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* WS_AGENT_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to WS_AGENT_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("ws-agent-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
