...
function stripUnknownConfigKeys(config: WsAgentConfig): {
  config: WsAgentConfig;
  removed: string[];
} {
  const parsed = WsAgentSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: WsAgentConfig) {
...
function scanTelegramAllowFromUsernameEntries(cfg: WsAgentConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: WsAgentConfig): Promise<{
  config: WsAgentConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: WsAgentConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: WsAgentConfig): {
  config: WsAgentConfig;
  changes: string[];
} {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: WsAgentConfig): {
  config: WsAgentConfig;
  changes: string[];
} {
...
function collectExecSafeBinScopes(cfg: WsAgentConfig): ExecSafeBinScopeRef[] {
...
function scanExecSafeBinCoverage(cfg: WsAgentConfig): ExecSafeBinCoverageHit[] {
...
function maybeRepairExecSafeBinProfiles(cfg: WsAgentConfig): {
  config: WsAgentConfig;
  changes: string[];
  warnings: string[];
} {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".ws-agent");
  const targetPath = path.join(targetDir, "ws-agent.json");
  try {
  ...
  const legacyCandidates = [
    path.join(home, ".ws-agent", "ws-agent.json"),
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("ws-agent doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("ws-agent doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (shouldRepair) {
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("ws-agent doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("ws-agent doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("ws-agent doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (safeBinCoverage.length > 0) {
      ...
      lines.push(
        `- Run "${formatCliCommand("ws-agent doctor --fix")}" to scaffold missing custom safeBinProfiles entries.`,
      );
      note(lines.join("\n"), "Doctor warnings");
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "ws-agent doctor --fix" to remove these keys.');
    }
  }
