...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.wsAgentCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.wsAgentCanvasA2UIAction &&
        typeof window.wsAgentCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.wsAgentSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("ws-agent:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android WsAgent node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.wsAgentSendUserAction === "function"
        ? window.wsAgentSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.WS_AGENT_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.WS_AGENT_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>WsAgent Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
