...
describe("canvas host state dir defaults", () => {
  it("uses WS_AGENT_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("ws-agent-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("WsAgent Canvas");
      } finally {
        await handler.close();
      }
    });
