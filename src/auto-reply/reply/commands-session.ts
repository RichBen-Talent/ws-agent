...
import { scheduleGatewaySigusr1Restart, triggerWsAgentRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting WsAgent in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerWsAgentRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting WsAgent via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
