...
export function isCommandEnabled(cfg: WsAgentConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: WsAgentConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as WsAgentConfig),
...
export function getCommandDetection(_cfg?: WsAgentConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: WsAgentConfig) {
