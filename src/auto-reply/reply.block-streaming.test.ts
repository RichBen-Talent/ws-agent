...
function createReplyConfig(home: string, streamMode?: "block"): WsAgentConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("ws-agent-stream-", async (home) => {
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("WS_AGENT_TEST_FAST", "1");
