...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "ws-agent-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "ws-agent-bundled-skills-test-"));
    process.env.WS_AGENT_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
