...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update WsAgent and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["ws-agent update", "Update a source checkout (git)"],
        ["ws-agent update --channel beta", "Switch to beta channel (git + npm)"],
        ["ws-agent update --channel dev", "Switch to dev channel (git + npm)"],
        ["ws-agent update --tag beta", "One-off update to a dist-tag or version"],
        ["ws-agent update --no-restart", "Update without restarting the service"],
        ["ws-agent update --json", "Output result as JSON"],
        ["ws-agent update --yes", "Non-interactive (accept downgrade prompts)"],
        ["ws-agent update wizard", "Interactive update wizard"],
        ["ws-agent --update", "Shorthand for ws-agent update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run ws-agent update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.ws-agent.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["ws-agent update status", "Show channel + version status."],
          ["ws-agent update status --json", "JSON output."],
          ["ws-agent update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.ws-agent.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
