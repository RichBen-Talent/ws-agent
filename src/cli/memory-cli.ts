    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["ws-agent memory status", "Show index and provider status."],
          ["ws-agent memory index --force", "Force a full reindex."],
          ['ws-agent memory search --query "deployment notes"', "Search indexed memory entries."],
          ["ws-agent memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.ws-agent.ai/cli/memory")}\n`,
    );
