...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("ws-agent node install"), formatCliCommand("ws-agent node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("ws-agent node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    WS_AGENT_LOG_PREFIX: baseEnv.WS_AGENT_LOG_PREFIX ?? "node",
