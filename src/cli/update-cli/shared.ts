...
const WS_AGENT_REPO_URL = "https://github.com/ws-agent/ws-agent.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "ws-agent";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("ws-agent@")) {
    return trimmed.slice("ws-agent@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.WS_AGENT_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveWsAgentPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", WS_AGENT_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `WS_AGENT_GIT_DIR points at a non-git directory: ${params.dir}. Set WS_AGENT_GIT_DIR to an empty folder or an ws-agent checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`WS_AGENT_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "ws-agent.mjs");
