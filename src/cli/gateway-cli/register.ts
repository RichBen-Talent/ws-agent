      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["ws-agent gateway run", "Run the gateway in the foreground."],
            ["ws-agent gateway status", "Show service status and probe reachability."],
            ["ws-agent gateway discover", "Find local and wide-area gateway beacons."],
            ["ws-agent gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.ws-agent.ai/cli/gateway")}\n`,
      ),
  );
