...
import type { ConfigFileSnapshot, WsAgentConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `ws-agent config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: WsAgentConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: WsAgentConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: WsAgentConfig, config: WsAgentConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: WsAgentConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
  ...
  describe("config set parsing flags", () => {
    it("falls back to raw string when parsing fails and strict mode is off", async () => {
      const resolved: WsAgentConfig = { gateway: { port: 18789 } };
