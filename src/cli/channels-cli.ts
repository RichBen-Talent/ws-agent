    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["ws-agent channels list", "List configured channels and auth profiles."],
          ["ws-agent channels status --probe", "Run channel status checks and probes."],
          [
            "ws-agent channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["ws-agent channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.ws-agent.ai/cli/channels",
        )}\n`,
    );
