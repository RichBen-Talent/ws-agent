...
describe("registerSubCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.WS_AGENT_DISABLE_LAZY_SUBCOMMANDS;
  ...
  it("registers only the primary placeholder and dispatches", async () => {
    const program = createRegisteredProgram(["node", "ws-agent", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "ws-agent"]);
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "ws-agent", "nodes", "list"], "ws-agent");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "ws-agent", "acp", "--help"], "ws-agent");
    ...
    await program.parseAsync(["node", "ws-agent", "acp"], { from: "user" });
    expect(registerAcpCli).toHaveBeenCalledTimes(1);
    expect(acpAction).toHaveBeenCalledTimes(1);
  });
});
