...
${formatHelpExamples([
  ['ws-agent agent --to +15555550123 --message "status update"', "Start a new session."],
  ['ws-agent agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'ws-agent agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'ws-agent agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['ws-agent agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'ws-agent agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.ws-agent.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.ws-agent.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['ws-agent agents set-identity --agent main --name "WsAgent" --emoji "🦞"', "Set name + emoji."],
  ["ws-agent agents set-identity --agent main --avatar avatars/ws-agent.png", "Set avatar path."],
  [
    "ws-agent agents set-identity --workspace ~/.ws-agent/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "ws-agent agents set-identity --identity-file ~/.ws-agent/workspace/IDENTITY.md --agent main",
