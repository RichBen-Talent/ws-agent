...
beforeEach(() => {
  ...
  originalHideBanner = process.env.WS_AGENT_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.WS_AGENT_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.WS_AGENT_HIDE_BANNER = originalHideBanner;
  }
});

describe("registerPreActionHooks", () => {
  function buildProgram() {
    const program = new Command().name("ws-agent");
  ...
  it("emits banner, resolves config, and enables verbose from --debug", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "ws-agent", "status", "--debug"],
    });
  ...
  it("skips config guard for doctor and completion commands", async () => {
    await runCommand({
      parseArgv: ["doctor"],
      processArgv: ["node", "ws-agent", "doctor"],
    });
    await runCommand({
      parseArgv: ["completion"],
      processArgv: ["node", "ws-agent", "completion"],
    });
  ...
  it("skips preaction work when argv indicates help/version", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "ws-agent", "--version"],
    });
  ...
  it("hides banner when WS_AGENT_HIDE_BANNER is truthy", async () => {
    process.env.WS_AGENT_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "ws-agent", "status"],
    });
