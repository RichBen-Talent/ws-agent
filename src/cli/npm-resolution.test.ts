...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@ws-agent/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@ws-agent/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@ws-agent/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@ws-agent/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @ws-agent/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@ws-agent/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@ws-agent/plugin-alpha@1.2.3",
        installPath: "/tmp/ws-agent/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@ws-agent/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @ws-agent/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@ws-agent/plugin-alpha@latest",
      true,
      "/tmp/ws-agent/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@ws-agent/plugin-alpha@latest",
      installPath: "/tmp/ws-agent/extensions/alpha",
      version: "1.2.3",
