...
describe("gateway-cli coverage", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      expectedOutput: [
        "Gateway Discovery",
        "Found 1 gateway(s)",
        "- Studio ws-agent.internal.",
        "  tailnet: studio.tailnet.ts.net",
        "  host: studio.ws-agent.internal",
        "  ws: ws://studio.ws-agent.internal:18789",
      ],
    },
  ...
  it("uses env/config port when --port is omitted", async () => {
    await withEnvOverride({ WS_AGENT_GATEWAY_PORT: "19001" }, async () => {
      resetRuntimeCapture();
