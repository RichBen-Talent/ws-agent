...
import type { WsAgentConfig, ConfigFileSnapshot } from "../config/types.ws-agent.js";
...
vi.mock("../infra/ws-agent-root.js", () => ({
  resolveWsAgentPackageRoot: vi.fn(),
}));
...
const { resolveWsAgentPackageRoot } = await import("../infra/ws-agent-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/ws-agent-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as WsAgentConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/ws-agent-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveWsAgentPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("ws-agent-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveWsAgentPackageRoot).mockClear();
    ...
    vi.mocked(resolveWsAgentPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("ws-agent");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/ws-agent-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "ws-agent-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("WsAgent update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as WsAgentConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveWsAgentPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("ws-agent-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ WS_AGENT_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.WS_AGENT_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("ws-agent-update-wizard");
    await withEnvAsync({ WS_AGENT_GIT_DIR: tempDir }, async () => {
      setTty(true);
