...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.ws-agent.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + WsAgent hooks")
    ...
    .option("--hook-url <url>", "WsAgent hook URL")
    .option("--hook-token <token>", "WsAgent hook token")
