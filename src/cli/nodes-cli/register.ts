    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["ws-agent nodes status", "List known nodes with live status."],
          ["ws-agent nodes pairing pending", "Show pending node pairing requests."],
          ['ws-agent nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["ws-agent nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.ws-agent.ai/cli/nodes")}\n`,
    );
