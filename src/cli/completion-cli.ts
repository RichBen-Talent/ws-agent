...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# WsAgent Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(ws-agent completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# WsAgent Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(ws-agent completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option(
      "--write-state",
      "Write completion scripts to $WS_AGENT_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "ws-agent") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c ws-agent -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'ws-agent gateway' and not 'ws-agent other gateway', we need to check the sequence.
