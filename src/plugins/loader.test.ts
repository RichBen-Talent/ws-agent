...
import { loadWsAgentPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `ws-agent-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.WS_AGENT_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "ws-agent.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.WS_AGENT_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadWsAgentPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.WS_AGENT_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.WS_AGENT_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadWsAgentPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadWsAgentPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadWsAgentPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.WS_AGENT_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ WS_AGENT_STATE_DIR: stateDir }, () => {
