...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: WsAgentPluginDefinition;
  register?: WsAgentPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as WsAgentPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as WsAgentPluginDefinition;
...
export function loadWsAgentPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverWsAgentPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "ws-agent/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "ws-agent/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: WsAgentPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as WsAgentPluginModule;
    } catch (err) {
