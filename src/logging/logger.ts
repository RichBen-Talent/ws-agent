...
import type { WsAgentConfig } from "../config/types.js";
...
export const DEFAULT_LOG_DIR = resolvePreferredWsAgentTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "ws-agent.log"); // legacy single-file path

const LOG_PREFIX = "ws-agent";
...
function resolveSettings(): ResolvedSettings {
  let cfg: WsAgentConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => WsAgentConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.WS_AGENT_TEST_FILE_LOG !== "1" ? "silent" : "info";
