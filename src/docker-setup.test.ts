...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "ws-agent-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    WS_AGENT_GATEWAY_TOKEN: "test-token",
    WS_AGENT_CONFIG_DIR: join(sandbox.rootDir, "config"),
    WS_AGENT_WORKSPACE_DIR: join(sandbox.rootDir, "ws-agent"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        WS_AGENT_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
        WS_AGENT_EXTRA_MOUNTS: undefined,
        WS_AGENT_HOME_VOLUME: "ws-agent-home",
      }),
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("WS_AGENT_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("WS_AGENT_EXTRA_MOUNTS=");
    expect(envFile).toContain("WS_AGENT_HOME_VOLUME=ws-agent-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("ws-agent-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("ws-agent-home:");
    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg WS_AGENT_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });

  it("rejects injected multiline WS_AGENT_EXTRA_MOUNTS values", async () => {
    if (!sandbox) {
      throw new Error("sandbox missing");
    }
    ...
    expect(result.stderr).toContain("WS_AGENT_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid WS_AGENT_EXTRA_MOUNTS mount format", async () => {
    if (!sandbox) {
      throw new Error("sandbox missing");
    }
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      cwd: sandbox.rootDir,
      env: createEnv(sandbox, {
        WS_AGENT_EXTRA_MOUNTS: "bad mount spec",
      }),
      encoding: "utf8",
      stdio: ["ignore", "ignore", "pipe"],
    });
  ...
  it("rejects invalid WS_AGENT_HOME_VOLUME names", async () => {
    if (!sandbox) {
      throw new Error("sandbox missing");
    }
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("WS_AGENT_HOME_VOLUME must match");
  });
