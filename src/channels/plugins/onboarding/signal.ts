...
function setSignalDmPolicy(cfg: WsAgentConfig, dmPolicy: DmPolicy) {
...
export const signalOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    await prompter.note(
      [
        'Link device with: signal-cli link -n "WsAgent"',
        "Scan QR in Signal → Linked Devices",
        `Then run: ${formatCliCommand("ws-agent gateway call channels.status --params '{\"probe\":true}'")}`,
        `Docs: ${formatDocsLink("/signal", "signal")}`,
      ].join("\n"),
      "Signal next steps",
    );
