...
function telegramCfg(): WsAgentConfig {
  return { channels: { telegram: { botToken: "tok" } } } as WsAgentConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: WsAgentConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as WsAgentConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as WsAgentConfig;
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as WsAgentConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as WsAgentConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as WsAgentConfig,
        accountId: undefined,
        params: {
