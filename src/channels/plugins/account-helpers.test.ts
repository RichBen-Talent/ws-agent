...
function cfg(accounts?: Record<string, unknown> | null): WsAgentConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as WsAgentConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as WsAgentConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as WsAgentConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as WsAgentConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as WsAgentConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as WsAgentConfig)).toBe("default");
    });
  });
});
