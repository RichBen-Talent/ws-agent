...
function resolveGatewayPort(cfg: WsAgentConfig, env: NodeJS.ProcessEnv): number {
...
function resolveAuth(cfg: WsAgentConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  ...
  const password =
    env.WS_AGENT_GATEWAY_PASSWORD?.trim() ||
    env.WS_AGENT_GATEWAY_PASSWORD?.trim() ||
