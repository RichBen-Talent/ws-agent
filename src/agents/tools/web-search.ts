...
type WebSearchConfig = NonNullable<WsAgentConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: WsAgentConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("ws-agent configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.ws-agent.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://ws-agent.ai",
      "X-Title": "WsAgent Web Search",
    },
