...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): WsAgentConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as WsAgentConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): WsAgentConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as WsAgentConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: WsAgentConfig, accountId = "media") {
    await handleTelegramAction(
