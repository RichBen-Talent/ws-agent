...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("ws-agent-ws-", async (workspaceDir) => {
      await withTempDir("ws-agent-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createWsAgentCodingTools({ workspaceDir });
    ...
    await withTempDir("ws-agent-ws-", async (workspaceDir) => {
      await withTempDir("ws-agent-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello ws-agent");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("ws-agent-ws-", async (workspaceDir) => {
      await withTempDir("ws-agent-override-", async (overrideDir) => {
        const tools = createWsAgentCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("ws-agent-sandbox-", async (sandboxDir) => {
      await withTempDir("ws-agent-workspace-", async (workspaceDir) => {
        ...
        const tools = createWsAgentCodingTools({ workspaceDir, sandbox });
