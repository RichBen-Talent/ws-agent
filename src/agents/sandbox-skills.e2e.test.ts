...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "ws-agent-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "ws-agent-bundled-skills-"));
    await fs.mkdir(bundledDir, { recursive: true });

    process.env.WS_AGENT_BUNDLED_SKILLS_DIR = bundledDir;
