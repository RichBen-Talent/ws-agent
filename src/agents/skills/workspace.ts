...
function resolveSkillsLimits(config?: WsAgentConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "ws-agent-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "ws-agent-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveWsAgentMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: WsAgentConfig }): {
...
export function buildWorkspaceSkillSnapshot(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`ws-agent skills check\` to audit.`
    : "";
