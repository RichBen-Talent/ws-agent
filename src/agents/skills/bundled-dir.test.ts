...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["WS_AGENT_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns WS_AGENT_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "ws-agent-bundled-override-"));
    process.env.WS_AGENT_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
    expect(resolveBundledSkillsDir()).toBe(overrideDir);
  });

  it("resolves bundled skills under a flattened dist layout", async () => {
    delete process.env.WS_AGENT_BUNDLED_SKILLS_DIR;

    const root = await fs.mkdtemp(path.join(os.tmpdir(), "ws-agent-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "ws-agent" }));
