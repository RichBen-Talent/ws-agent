...
const defaultTools = createWsAgentCodingTools({ senderIsOwner: true });
...
describe("createWsAgentCodingTools", () => {
  ...
  it("gates apply_patch behind tools.exec.applyPatch for OpenAI models", () => {
    ...
    const openAiTools = createWsAgentCodingTools({
      config,
    ...
    const anthropicTools = createWsAgentCodingTools({
      config,
  ...
  it("respects apply_patch allowModels", () => {
    ...
    const allowed = createWsAgentCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    ...
    const denied = createWsAgentCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
