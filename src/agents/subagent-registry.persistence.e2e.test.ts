...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["WS_AGENT_STATE_DIR"]);
  let tempStateDir: string | null = null;

  const writePersistedRegistry = async (persisted: Record<string, unknown>) => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "ws-agent-subagent-"));
  ...
  it("uses isolated temp state when WS_AGENT_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "ws-agent-test-state"));
  });
});
