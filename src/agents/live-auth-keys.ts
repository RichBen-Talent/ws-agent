...
const GOOGLE_LIVE_SINGLE_KEY = "WS_AGENT_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "WS_AGENT_LIVE_ANTHROPIC_KEY",
    listVar: "WS_AGENT_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "WS_AGENT_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `WS_AGENT_LIVE_${base}_KEY`;
