...
describe("workspace bootstrap file caching", () => {
  ...
  beforeEach(async () => {
    workspaceDir = await makeTempWorkspace("ws-agent-bootstrap-cache-test-");
  });
  ...
  it("caches files independently by path", async () => {
    ...
    const workspace1 = await makeTempWorkspace("ws-agent-cache-test1-");
    const workspace2 = await makeTempWorkspace("ws-agent-cache-test2-");
