...
function resolveMainSessionKeyForSandbox(params: { cfg?: WsAgentConfig; agentId: string }): string {
...
export function resolveSandboxRuntimeStatus(params: { cfg?: WsAgentConfig; sessionKey?: string }): {
  agentId: string;
  sessionKey: string;
...
export function formatSandboxToolPolicyBlockedMessage(params: {
  ...
  lines.push(
    `- See: ${formatCliCommand(`ws-agent sandbox explain --session ${runtime.sessionKey}`)}`,
  );
