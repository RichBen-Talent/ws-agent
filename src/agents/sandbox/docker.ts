...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "ws-agent.sandbox=1");
  args.push("--label", `ws-agent.sessionKey=${params.scopeKey}`);
  args.push("--label", `ws-agent.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `ws-agent.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "ws-agent.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`ws-agent sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`ws-agent sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("ws-agent sandbox recreate --all");
}
