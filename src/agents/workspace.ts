...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.WS_AGENT_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".ws-agent", `workspace-${profile}`);
  }
  return path.join(home, ".ws-agent", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".ws-agent";
