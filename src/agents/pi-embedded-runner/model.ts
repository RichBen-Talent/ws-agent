...
export function resolveModel(
  ...
  const resolvedAgentDir = agentDir ?? resolveWsAgentAgentDir();
 ...
 * a hint.
 *
 * See: https://github.com/ws-agent/ws-agent/issues/17328
 */
const LOCAL_PROVIDER_HINTS: Record<string, string> = {
  ollama:
    "Ollama requires authentication to be registered as a provider. " +
    'Set OLLAMA_API_KEY="ollama-local" (any value works) or run "ws-agent configure". ' +
    "See: https://docs.ws-agent.ai/providers/ollama",
  vllm:
    "vLLM requires authentication to be registered as a provider. " +
    'Set VLLM_API_KEY (any value works) or run "ws-agent configure". ' +
    "See: https://docs.ws-agent.ai/providers/vllm",
};
