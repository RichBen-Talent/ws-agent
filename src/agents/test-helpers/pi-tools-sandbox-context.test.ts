...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("ws-agent-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("ws-agent-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("ws-agent-sbx-");
