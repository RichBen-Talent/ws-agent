...
type CreateWsAgentTools = (typeof import("./ws-agent-tools.js"))["createWsAgentTools"];
export type CreateWsAgentToolsOpts = Parameters<CreateWsAgentTools>[0];
...
export async function getSessionsSpawnTool(opts: CreateWsAgentToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createWsAgentTools } = await import("./ws-agent-tools.js");
  const tool = createWsAgentTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
