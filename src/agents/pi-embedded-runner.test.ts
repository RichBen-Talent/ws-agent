...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ws-agent-embedded-agent-"));
...
const ensureModels = (cfg: WsAgentConfig) => ensureWsAgentModelsJson(cfg, agentDir) as unknown;
