...
describe("resolveGatewayConnection", () => {
  ...
  it("uses WS_AGENT_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ WS_AGENT_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers WS_AGENT_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ WS_AGENT_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
