...
// Keep integration tests deterministic: never read a real ws-agent.json.
vi.mock("../config.js", () => ({
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ws-agent-pruning-integ-"));
  });
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.WS_AGENT_SESSION_CACHE_TTL_MS;
    process.env.WS_AGENT_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.WS_AGENT_SESSION_CACHE_TTL_MS;
    } else {
      process.env.WS_AGENT_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
