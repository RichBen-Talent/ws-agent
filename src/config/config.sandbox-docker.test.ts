...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("ws-agent-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "ws-agent-sandbox-browser-global" },
      agentBrowser: { network: "ws-agent-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("ws-agent-sandbox-browser-agent");
  });
