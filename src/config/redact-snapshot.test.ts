...
import type { ConfigFileSnapshot } from "./types.ws-agent.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.ws-agent/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/ws-agent/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/ws-agent/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/ws-agent/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/ws-agent/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = WsAgentSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "WsAgentConfig";
    const hints = mapSensitivePaths(WsAgentSchema, "", {});
