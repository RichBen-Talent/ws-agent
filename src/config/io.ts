...
import type { WsAgentConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareWsAgentVersions } from "./version.js";
...
function coerceConfig(value: unknown): WsAgentConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as WsAgentConfig;
}
...
function stampConfigVersion(cfg: WsAgentConfig): WsAgentConfig {
...
function warnIfConfigFromFuture(cfg: WsAgentConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareWsAgentVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer WsAgent (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function normalizeExecSafeBinProfilesInConfig(cfg: WsAgentConfig): void {
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as WsAgentConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): WsAgentConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as WsAgentConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: WsAgentConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as WsAgentConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.WS_AGENT_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.WS_AGENT_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.WS_AGENT_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.WS_AGENT_WATCH_SESSION === "string" &&
        deps.env.WS_AGENT_WATCH_SESSION.trim().length > 0
          ? deps.env.WS_AGENT_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.WS_AGENT_WATCH_COMMAND === "string" &&
        deps.env.WS_AGENT_WATCH_COMMAND.trim().length > 0
          ? deps.env.WS_AGENT_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `WS_AGENT_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.WS_AGENT_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.WS_AGENT_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): WsAgentConfig {
