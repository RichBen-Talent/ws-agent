...
import type { WsAgentConfig, ConfigValidationIssue } from "./types.js";
import { WsAgentSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: WsAgentConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: WsAgentConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = WsAgentSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as WsAgentConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as WsAgentConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as WsAgentConfig,
  };
}
