...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, "ws-agent_cmdarg", "arg-menu action");
    argMenuOptionsHandler = requireHandler(harness.options, "ws-agent_cmdarg", "arg-menu options");
  });
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(testHarness.actions.has("ws-agent_cmdarg")).toBe(true);
    expect(testHarness.options.has("ws-agent_cmdarg")).toBe(true);
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("ws-agent_cmdarg");
  ...
  it("shows an external_select menu when choices exceed static_select options max", async () => {
    ...
    expect(blockId).toContain("ws-agent_cmdarg_ext:");
    const token = (blockId ?? "").slice("ws-agent_cmdarg_ext:".length);
