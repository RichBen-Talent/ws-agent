...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const previousStateDir = process.env.WS_AGENT_STATE_DIR;

    const tempHome = await createTempHomeEnv("ws-agent-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.WS_AGENT_STATE_DIR).toBe(path.join(tempHome.home, ".ws-agent"));
    ...
    expect(process.env.WS_AGENT_STATE_DIR).toBe(previousStateDir);
