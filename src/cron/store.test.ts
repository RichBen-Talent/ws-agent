...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "ws-agent-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses WS_AGENT_HOME for tilde expansion", () => {
    vi.stubEnv("WS_AGENT_HOME", "/srv/ws-agent-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/ws-agent-home", "cron", "jobs.json"));
  });
});
