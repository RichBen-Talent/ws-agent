...
# Build and bundle WsAgent into a minimal .app we can open.
# Outputs to dist/WsAgent.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/WsAgent.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="WsAgent"
BUNDLE_ID="${BUNDLE_ID:-ai.wsagent.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/ws-agent/ws-agent/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/WsAgent/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :WsAgentBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :WsAgentGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/WsAgent"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/WsAgent"
fi
chmod +x "$APP_ROOT/Contents/MacOS/WsAgent"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/WsAgent" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/WsAgent/Resources/WsAgent.icns" "$APP_ROOT/Contents/Resources/WsAgent.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/WsAgent/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying WsAgentKit resources"
WS_AGENTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/WsAgentKit_WsAgentKit.bundle"
if [ -d "$WS_AGENTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/WsAgentKit_WsAgentKit.bundle"
  cp -R "$WS_AGENTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/WsAgentKit_WsAgentKit.bundle"
else
  echo "WARN: WsAgentKit resource bundle not found at $WS_AGENTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running WsAgent"
killall -q WsAgent 2>/dev/null || true
