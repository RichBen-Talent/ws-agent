...
#
# Suggested cron: */30 * * * * /home/admin/ws-agent/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send WsAgent notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., ws-agent-alerts)
...
STATE_FILE="$HOME/.ws-agent/auth-monitor-state"
...
send_notification() {
    ...
    # Send via WsAgent if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use ws-agent
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via WsAgent to $NOTIFY_PHONE..."
            ws-agent send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! WsAgent is down. Run: ssh l36 '~/ws-agent/scripts/mobile-reauth.sh'" "urgent"
    exit 1
