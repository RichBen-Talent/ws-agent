...
BASE_IMAGE="${BASE_IMAGE:-ws-agent-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-ws-agent-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=ws-agent.sandbox=1)
NOTE
