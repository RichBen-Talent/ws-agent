...
IMAGE_NAME="ws-agent-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="ws-agent-net-e2e-$$"
GW_NAME="ws-agent-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "WS_AGENT_GATEWAY_TOKEN=$TOKEN" \
	  -e "WS_AGENT_SKIP_CHANNELS=1" \
	  -e "WS_AGENT_SKIP_GMAIL_WATCHER=1" \
	  -e "WS_AGENT_SKIP_CRON=1" \
	  -e "WS_AGENT_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
