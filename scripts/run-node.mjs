...
const shouldBuild = (deps) => {
  if (deps.env.WS_AGENT_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.WS_AGENT_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[ws-agent] ${message}\n`);
};

const runWsAgent = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["ws-agent.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
