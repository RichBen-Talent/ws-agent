...
INSTALL_URL="${WS_AGENT_INSTALL_URL:-https://ws-agent.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${WS_AGENT_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${WS_AGENT_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="ws-agent"
PACKAGE_NAME="${WS_AGENT_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (WS_AGENT_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${WS_AGENT_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${WS_AGENT_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
