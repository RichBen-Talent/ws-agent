 ...
 * `ws-agent update`. Use it to verify the completion installation flow
...
\`ws-agent update\`, \`ws-agent doctor\`, and \`ws-agent onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# WsAgent Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`ws-agent completion --install\` later to enable.`));
    return;
  }
