#!/usr/bin/env bash
# Reset WsAgent like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${WS_AGENT_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="WsAgent.app/Contents/MacOS/WsAgent"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/WsAgent"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/WsAgent"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/WsAgent"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.wsagent.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/ws-agent-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${WS_AGENT_RESTART_LOG:-/tmp/ws-agent-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${WS_AGENT_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.ws-agent/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  WS_AGENT_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node ws-agent.mjs daemon install --force --runtime node"
      log "  node ws-agent.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.ws-agent/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.wsagent.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing WsAgent instances"
kill_all_ws-agent
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product WsAgent"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/WsAgent.app" ]]; then
    APP_BUNDLE="/Applications/WsAgent.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/WsAgent.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/WsAgent.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/WsAgent.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set WS_AGENT_APP_BUNDLE to your installed WsAgent.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node ws-agent.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node ws-agent.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".ws-agent", "ws-agent.json"), "utf8");
