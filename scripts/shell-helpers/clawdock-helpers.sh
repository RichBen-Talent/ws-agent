#!/usr/bin/env bash
# ClawDock - Docker helpers for WsAgent
# Inspired by Simon Willison's "Running WsAgent in Docker"
# https://til.simonwillison.net/llms/ws-agent-docker
#
# Installation:
...
# Common paths to check for WsAgent
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/ws-agent"
  "${HOME}/workspace/ws-agent"
  "${HOME}/projects/ws-agent"
  "${HOME}/dev/ws-agent"
  "${HOME}/code/ws-agent"
  "${HOME}/src/ws-agent"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found WsAgent at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/ws-agent"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ WsAgent not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/ws-agent/ws-agent.git ~/ws-agent"
    echo "  cd ~/ws-agent && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^WS_AGENT_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d ws-agent-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart ws-agent-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f ws-agent-gateway
}
...
clawdock-workspace() {
  cd ~/.ws-agent/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec ws-agent-gateway \
    bash -c 'echo "alias ws-agent=\"./ws-agent.mjs\"" > /tmp/.bashrc_ws-agent && bash --rcfile /tmp/.bashrc_ws-agent'
}
...
clawdock-cli() {
  _clawdock_compose run --rm ws-agent-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build ws-agent-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "WS_AGENT_GATEWAY_TOKEN=$token" ws-agent-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" ws-agent-gateway \
    bash -c './ws-agent.mjs config set gateway.remote.token "$TOKEN" && ./ws-agent.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec ws-agent-gateway \
    bash -c "./ws-agent.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart ws-agent-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm ws-agent-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec ws-agent-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'ws-agent config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for WsAgent${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (ws-agent alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to ws-agent project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.ws-agent)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'ws-agent channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'ws-agent status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.ws-agent.ai${_CLR_RESET}"
  echo ""
}
