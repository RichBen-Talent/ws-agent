...
# WsAgent Auth Widget for Termux
...
# Check auth status
termux-toast "Checking WsAgent auth..."

STATUS=$(ssh "$SERVER" '$HOME/ws-agent/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/ws-agent/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|WS_AGENT_EXPIRING|WS_AGENT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/ws-agent/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|WS_AGENT_EXPIRED|WS_AGENT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: WsAgent won't work until re-auth"
                ;;
        esac
        ;;
