...
ZIP=${1:?"Usage: $0 WsAgent-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/ws-agent/ws-agent/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^WsAgent-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/ws-agent/ws-agent/releases/download/v${VERSION}/"}
