#!/bin/bash
# Setup WsAgent Auth Management System
...
echo "=== WsAgent Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'ws-agent-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  WsAgent message: Send warning via WsAgent itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/ws-agent-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/ws-agent-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/ws-agent-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now ws-agent-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status ws-agent-auth-monitor.timer"
echo ""
echo "Quick commands:"
