...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/ws-agent/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "WsAgent Re-Auth" -c "After getting key, run: ssh $SERVER '~/ws-agent/scripts/mobile-reauth.sh'" --id ws-agent-auth
        ;;
    *)
