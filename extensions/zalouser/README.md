...
WsAgent extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
ws-agent channels login --channel zalouser
...
```bash
ws-agent message send --channel zalouser --target <threadId> --message "Hello from WsAgent!"
```
...
```bash
ws-agent channels login --channel zalouser              # Login via QR
ws-agent channels login --channel zalouser --account work
ws-agent channels status --probe
ws-agent channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
ws-agent directory self --channel zalouser
ws-agent directory peers list --channel zalouser --query "name"
ws-agent directory groups list --channel zalouser --query "work"
ws-agent directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
ws-agent message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
ws-agent message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `ws-agent channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
ws-agent channels login --channel zalouser --account work
ws-agent message send --channel zalouser --account work --target <id> --message "Hello"
