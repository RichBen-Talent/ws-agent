...
import { createDedupeCache, createPersistentDedupe } from "ws-agent/plugin-sdk";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  ...
  if (env.VITEST || env.NODE_ENV === "test") {
    return path.join(os.tmpdir(), ["ws-agent-vitest", String(process.pid)].join("-"));
  }
  return path.join(os.homedir(), ".ws-agent");
}
