...
function resolveWsAgentRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.WS_AGENT_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set WS_AGENT_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveWsAgentRoot(), "dist", "extensionAPI.js");
