/**
 * Twitch channel plugin for WsAgent.
 *
...
import { buildChannelConfigSchema } from "ws-agent/plugin-sdk";
 ...
 * for WsAgent. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: WsAgentConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: WsAgentConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: WsAgentConfig): boolean => {
