...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["wsAgentPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["ws-agent", "pollId"]) ??
    readNestedString(value, ["ws-agent", "poll", "id"]) ??
    readNestedString(value, ["data", "wsAgentPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "ws-agent", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          wsAgentPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "ws-agent poll vote",
          displayText: "Vote recorded",
          value: { wsAgentPollId: pollId, pollId },
        },
      },
    ],
  };
