...
`ws-agent.invoke --each`) without adding new WsAgent code per workflow.
...
This extension depends on WsAgent internal modules (the embedded agent runner).
It is intended to ship as a **bundled** WsAgent extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.ws-agent/extensions` as a standalone plugin directory.
