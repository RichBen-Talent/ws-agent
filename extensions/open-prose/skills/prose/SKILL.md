...
metadata: { "ws-agent": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == WsAgent `sessions_spawn`
- **File I/O** == WsAgent `read`/`write`
- **Remote fetch** == WsAgent `web_fetch` (or `exec` with curl when POST is required)
