...
import type { WsAgentPluginApi, WsAgentPluginService } from "ws-agent/plugin-sdk";
...
function normalizeDenyList(cfg: WsAgentPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: WsAgentPluginApi["config"]): string[] {
...
export default function register(api: WsAgentPluginApi) {
  ...
  const timerService: WsAgentPluginService = {
