...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as WsAgentConfig);
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as WsAgentConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as WsAgentConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as WsAgentConfig);
