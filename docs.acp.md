...
This document describes how the WsAgent ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`ws-agent acp` exposes an ACP agent over stdio and forwards prompts to a running
WsAgent Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `ws-agent acp` and speaks ACP messages over stdio.
...
`ws-agent acp` resolves the Gateway URL and auth from CLI flags or config:
