...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `ws-agent-e2e-${name}-`));
  const configDir = path.join(homeDir, ".ws-agent");
    ...
    async () => {
      ...
      const [hookResA, hookResB] = await Promise.all([
        postJson(
          ...
          { "x-ws-agent-token": gwA.hookToken },
        ),
        postJson(
          ...
          { "x-ws-agent-token": gwB.hookToken },
        ),
      ]);
