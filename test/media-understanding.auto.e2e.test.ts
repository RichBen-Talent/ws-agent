...
import type { WsAgentConfig } from "../src/config/config.js";
import { resolvePreferredWsAgentTmpDir } from "../src/infra/tmp-ws-agent-dir.js";
...
const makeTempDir = async (prefix: string) => {
  const baseDir = resolvePreferredWsAgentTmpDir();
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("ws-agent-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    await withEnvSnapshot(async () => {
      const binDir = await createTrackedTempDir(tempPaths, "ws-agent-bin-sherpa-");
      const modelDir = await createTrackedTempDir(tempPaths, "ws-agent-sherpa-model-");
      ...
      const cfg: WsAgentConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    await withEnvSnapshot(async () => {
      const binDir = await createTrackedTempDir(tempPaths, "ws-agent-bin-whispercpp-");
      const modelDir = await createTrackedTempDir(tempPaths, "ws-agent-whispercpp-model-");
  ...
  it("uses gemini CLI for images when available", async () => {
    await withEnvSnapshot(async () => {
      const binDir = await createTrackedTempDir(tempPaths, "ws-agent-bin-gemini-");
      ...
      const cfg: WsAgentConfig = { tools: { media: { image: {} } } };
  ...
  it("skips auto-detect when no supported binaries are available", async () => {
    await withEnvSnapshot(async () => {
      const emptyBinDir = await createTrackedTempDir(tempPaths, "ws-agent-bin-empty-");
