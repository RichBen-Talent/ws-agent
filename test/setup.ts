...
process.env.WS_AGENT_PLUGIN_MANIFEST_CACHE_MS ??= "60000";
...
// Set HOME/state isolation before importing any runtime WsAgent modules.
...
const createStubPlugin = (params: {
  ...
  config: {
    listAccountIds: (cfg: WsAgentConfig) => {
    ...
    resolveAccount: (cfg: WsAgentConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: WsAgentConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
