...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    wsAgentHome: process.env.WS_AGENT_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("WS_AGENT_HOME", snapshot.wsAgentHome);
  restoreKey("WS_AGENT_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked WS_AGENT_HOME.
  delete process.env.WS_AGENT_HOME;
  process.env.WS_AGENT_STATE_DIR = path.join(base, ".ws-agent");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "ws-agent-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".ws-agent", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
